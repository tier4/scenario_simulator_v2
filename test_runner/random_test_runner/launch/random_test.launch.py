...
class RandomTestRunnerLaunch(object):
    def __init__(self):
        ...
        self.autoware_launch_arguments = {
            # autoware arguments #
            "architecture_type": {
                "default": "awf/universe/20230906",
                "description": "Autoware architecture type",
                "values": ["awf/universe/20230906", "awf/universe/20240605"],
            },
            "sensor_model": {
                "default": "sample_sensor_kit",
                "description": "Ego sensor model",
            },
            "vehicle_model": {
                "default": "sample_vehicle",
                "description": "Ego vehicle model",
            },
            "autoware_launch_file": {
                "default": "planning_simulator.launch.xml",
                "description": "Launch file name for Autoware running",
            },
            "autoware_launch_package": {
                "default": "autoware_launch",
                "description": "Launch file package name for Autoware running",
            },
        }
        ...
        self.random_test_arguments = {
            ...
            "simulator_type": {
                "default": "simple_sensor_simulator",
            ...
            "test_count": {
                "default": 5,
                "description": "Test count to be performed in test suite",
            },
            "test_timeout": {
                "default": 60.0,
                "description": "Timeout of the single test",
            },
            "input_dir": {
                "default": "",
            ...
            "initialize_duration": {
                "default": 35,
                "description": "How long test runner will wait for Autoware to initialize",
            },
            ...
            "npc_min_speed": {
                "default": 0.5,
                "description": "Minimum speed of generated npcs",
            },
            "npc_max_speed": {
                "default": 3.0,
                "description": "Maximum speed of generated npcs",
            },
            "npc_min_distance_to_ego": {
                "default": 10.0,
                "description": "Minimum distance of generated npcs from ego",
            },
            "npc_max_distance_to_ego": {
                "default": 100.0,
            ...
            "seed": {
                "default": -1,
                "description": "Randomization seed. If -1, seed will be generated for each test",
            },
        }
    ...
    def collect_random_test_launch_configuration(self):
        for (
            argument_name,
            argument_default_and_description,
        ) in self.random_test_arguments.items():
            self.random_test_runner_launch_configuration[
                argument_name
            ] = LaunchConfiguration(
                argument_name, default=argument_default_and_description["default"]
            )

        for (
            argument_name,
            argument_default_and_description,
        ) in self.autoware_launch_arguments.items():
            self.autoware_launch_configuration[argument_name] = LaunchConfiguration(
                argument_name, default=argument_default_and_description["default"]
            )
    ...
    def generate_launch_arguments_declarations(self):
        ...
        for (
            configuration_name,
            configuration_value,
        ) in self.random_test_runner_launch_configuration.items():
            declared_launch_arguments.append(
                self.create_declared_launch_argument(
                    configuration_name, self.random_test_arguments[configuration_name]
                )
            )
        ...
        for (
            configuration_name,
            configuration_value,
        ) in self.autoware_launch_configuration.items():
            declared_launch_arguments.append(
                self.create_declared_launch_argument(
                    configuration_name,
    ...
    def launch_setup(self, context, *args, **kwargs):
        ...
        if test_param_file:
            test_param_file_path = os.path.join(
                get_package_share_directory("random_test_runner"),
                "param",
                test_param_file,
            )
            print(
        ...
        if "awf/universe" in autoware_architecture:
            vehicle_model = self.autoware_launch_configuration["vehicle_model"].perform(
                context
            )
            if vehicle_model:
                ...
                vehicle_info_param_file_path = os.path.join(
                    vehicle_model_description_dir, "config/vehicle_info.param.yaml"
                )
                simulator_model_param_file_path = os.path.join(
                    vehicle_model_description_dir, "config/simulator_model.param.yaml"
                )

                print(
                    "Vehicle info parameters file supplied: '{}'. "
                    "Parameters passed there override passed via arguments".format(
                        vehicle_info_param_file_path
                    )
                )
                print(
                    "Simulator model parameters file supplied: '{}'. "
                    "Parameters passed there override passed via arguments".format(
