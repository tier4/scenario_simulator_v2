...
def on_stderr_output(event: launch.Event) -> None:
    ...
    if len(lines) == 1:
        if lines[0] == "context_gamma_scenario:failure":
...
def on_stdout_output(event: launch.Event) -> None:
    lines = event.text.decode().splitlines()
    if len(lines) == 1:
        if lines[0] == "context_gamma_scenario:success":
...
def launch_setup(context, *args, **kwargs):
    ...
    scenario_package                    = LaunchConfiguration("package",                                default="context_gamma_scenarios")
    ...
    context_gamma_scenario_node = Node(
    ...
    io_handler = OnProcessIO(
        target_action=context_gamma_scenario_node,
        on_stderr=on_stderr_output,
        on_stdout=on_stdout_output,
    )
    shutdown_handler = OnProcessExit(
        target_action=context_gamma_scenario_node, on_exit=[EmitEvent(event=Shutdown())]
    )
