...
{
public:
  explicit DefineTrafficSourceLarge(const rclcpp::NodeOptions & option)
    ...
    if (api_.getCurrentTime() >= 3.0) {
      ...
      for (const auto & name : names) {
        if (const auto entity = api_.getEntity(name)) {
          if (!entity->laneMatchingSucceed() || !isVehicle(name)) {
            stop(cpp_mock_scenarios::Result::FAILURE);  // LCOV_EXCL_LINE
          }
        }
      }
...
int main(int argc, char * argv[])
{
  ...
  auto component = std::make_shared<cpp_mock_scenarios::DefineTrafficSourceLarge>(options);
