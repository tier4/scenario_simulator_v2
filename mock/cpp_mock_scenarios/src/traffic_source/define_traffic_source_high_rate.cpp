...
{
class DefineTrafficSourceHighRate : public cpp_mock_scenarios::CppScenarioNode
{
public:
  explicit DefineTrafficSourceHighRate(const rclcpp::NodeOptions & option)
...
private:
  ...
  void onUpdate() override
  {
    if (api_.getCurrentTime() >= 3.0) {
      ...
      if (names.size() != static_cast<std::size_t>(150)) {
...
int main(int argc, char * argv[])
{
  ...
  auto component = std::make_shared<cpp_mock_scenarios::DefineTrafficSourceHighRate>(options);
