...
{
class DefineTrafficSourceDelay : public cpp_mock_scenarios::CppScenarioNode
{
public:
  explicit DefineTrafficSourceDelay(const rclcpp::NodeOptions & option)
...
private:
  ...
  {
    if (
      api_.getCurrentTime() < 2.0 && api_.getEntityNames().size() != static_cast<std::size_t>(1)) {
    ...
    if (api_.getCurrentTime() > 4.0) {
      ...
      // remove Ego, because we want to test entities spawned by traffic source
      names.erase(std::find(names.begin(), names.end(), "ego"));

      if (names.size() != static_cast<std::size_t>(4)) {
        stop(cpp_mock_scenarios::Result::FAILURE);  // LCOV_EXCL_LINE
      }
...
int main(int argc, char * argv[])
{
  ...
  auto component = std::make_shared<cpp_mock_scenarios::DefineTrafficSourceDelay>(options);
