...
namespace math {
namespace geometry {
auto CatmullRomSpline::getPolygon(const double width, const size_t num_points,
  ...
  std::vector<geometry_msgs::msg::Point> left_bounds =
      getLeftBounds(width, num_points, z_offset);
  std::vector<geometry_msgs::msg::Point> right_bounds =
      getRightBounds(width, num_points, z_offset);
...
auto CatmullRomSpline::getRightBounds(
    ...
    const double z_offset) const -> std::vector<geometry_msgs::msg::Point> {
  std::vector<geometry_msgs::msg::Point> points;
  double step_size = getLength() / static_cast<double>(num_points);
  for (size_t i = 0; i < num_points + 1; i++) {
    double s = step_size * static_cast<double>(i);
    points.emplace_back([this](const double local_width, const double local_s,
                               const double local_z_offset) {
...
auto CatmullRomSpline::getTrajectory(
    ...
    const double offset) const -> std::vector<geometry_msgs::msg::Point> {
  if (start_s > end_s) {
...
CatmullRomSpline::CatmullRomSpline(
    ...
    : control_points(control_points),
      line_segments_(getLineSegments(control_points)), total_length_(0) {
  switch (control_points.size()) {
  case 0:
    THROW_SEMANTIC_ERROR(
        "Control points are empty. We cannot determine the shape of the curve.",
        "This message is not originally intended to be displayed, if you see "
        "it, please contact "
  ...
  default:
    [this](const auto &control_points) -> void {
      ...
      for (size_t i = 0; i < n; i++) {
        if (i == 0) {
          double ax = 0;
          double bx = control_points[0].x - 2 * control_points[1].x +
                      control_points[2].x;
          double cx = -3 * control_points[0].x + 4 * control_points[1].x -
                      control_points[2].x;
          double dx = 2 * control_points[0].x;
          double ay = 0;
          double by = control_points[0].y - 2 * control_points[1].y +
                      control_points[2].y;
          double cy = -3 * control_points[0].y + 4 * control_points[1].y -
                      control_points[2].y;
          double dy = 2 * control_points[0].y;
          double az = 0;
          double bz = control_points[0].z - 2 * control_points[1].z +
                      control_points[2].z;
          double cz = -3 * control_points[0].z + 4 * control_points[1].z -
                      control_points[2].z;
...
auto CatmullRomSpline::getCurveIndexAndS(const double s) const
...
auto CatmullRomSpline::getSInSplineCurve(const size_t curve_index,
...
auto CatmullRomSpline::getCollisionPointsIn2D(
    ...
    const bool search_backward) const -> std::set<double> {
  if (polygon.size() <= 1) {
    THROW_SIMULATION_ERROR("Number of points in polygon are invalid, it "
                           "requires more than 2 points but only ",
                           static_cast<int>(polygon.size()), " exists.",
                           " This message is not originally intended to be "
                           "displayed, if you see it, please contact the "
  ...
  const auto get_collision_point_2d_with_curve =
      [this](const auto &local_polygon, const auto local_search_backward) {
        ...
        for (size_t i = 0; i < curves_.size(); ++i) {
          ...
          const auto s = curves_[i].getCollisionPointsIn2D(
              local_polygon, local_search_backward, true, true);
          std::for_each(s.begin(), s.end(),
                        [&s_value_candidates, i, this](const auto &s) {
  ...
  const auto get_collision_point_2d_with_line = [this](
                                                    const auto &local_polygon) {
    std::set<double> s_value_candidates;
    for (const auto &line : getLineSegments(local_polygon)) {
      if (static_cast<int>(line_segments_.size()) != 1) {
        THROW_SIMULATION_ERROR("Number of the line segments are invalid : ",
                               static_cast<int>(line_segments_.size()),
                               "Number of the line segments should be 1 when "
                               "the spline have 2 control points.",
                               "Something completely unexpected happened.",
                               "This message is not originally intended to be "
                               "displayed, if you see it, please "
                               "contact the developer of traffic_simulator.");
      }
      if (const auto s =
              line_segments_[0].get2DIntersectionSValue(line, true)) {
 ...
 * @param search_backward If true, return collision points with maximum s value.
 * If false, return collision points with minimum s value.
 * @return std::optional<double> Denormalized s values along the frenet
 * coordinate of the spline curve.
 */
auto CatmullRomSpline::getCollisionPointIn2D(
    const std::vector<geometry_msgs::msg::Point> &polygon,
    const bool search_backward) const -> std::optional<double> {
  std::set<double> s_value_candidates =
      getCollisionPointsIn2D(polygon, search_backward);
...
auto CatmullRomSpline::getCollisionPointIn2D(
  ...
  if (search_backward) {
    for (size_t i = 0; i < n; i++) {
      auto s = curves_[n - 1 - i].getCollisionPointIn2D(point0, point1,
...
auto CatmullRomSpline::getSValue(const geometry_msgs::msg::Pose &pose,
                                 const double threshold_distance) const
    -> std::optional<double> {
  switch (control_points.size()) {
  case 0:
    THROW_SIMULATION_ERROR("Only 0 control point exists for the spline.",
                           "If this message was displayed, something "
                           "completely unexpected happens.",
                           "This message is not originally intended to be "
                           "displayed, if you see it, please "
                           "contact the developer of traffic_simulator.");
  case 1:
    if (std::pow(control_points[0].x - pose.position.x, 2) +
  ...
  case 2:
    if (static_cast<int>(line_segments_.size()) != 1) {
      THROW_SIMULATION_ERROR("Number of the line segments are invalid : ",
                             static_cast<int>(line_segments_.size()),
                             "Number of the line segments should be 1 when the "
                             "spline have 2 control points.",
                             "Something completely unexpected happened.",
                             "This message is not originally intended to be "
                             "displayed, if you see it, please "
                             "contact the developer of traffic_simulator.");
...
auto CatmullRomSpline::getSquaredDistanceIn2D(
  ...
  case 1:
    ...
    THROW_SIMULATION_ERROR("Only 1 control point exists for the spline.",
...
auto CatmullRomSpline::getSquaredDistanceVector(
    ...
    const double s) const -> geometry_msgs::msg::Vector3 {
  switch (control_points.size()) {
  case 0:
    THROW_SIMULATION_ERROR("Only 0 control point exists for the spline.",
                           "If this message was displayed, something "
...
auto CatmullRomSpline::getPoint(const double s) const
...
auto CatmullRomSpline::getMaximum2DCurvature() const -> double {
  if (maximum_2d_curvatures_.empty()) {
    THROW_SIMULATION_ERROR(
        "maximum 2D curvature vector size is 0."); // LCOV_EXCL_LINE
  }
  const auto [min, max] = std::minmax_element(maximum_2d_curvatures_.begin(),
                                              maximum_2d_curvatures_.end());
...
auto CatmullRomSpline::getNormalVector(const double s) const
...
auto CatmullRomSpline::getTangentVector(const double s) const
    -> geometry_msgs::msg::Vector3 {
  switch (control_points.size()) {
  case 0:
    THROW_SIMULATION_ERROR("Only 0 control point exists for the spline.",
  ...
  default:
    /**
     * @note The current implementation uses `index_and_s` instead of structured
     * binding
     * (`const auto [index, s_value] = getCurveIndexAndS(s)`) because some tests
     * fail when using structured binding. The root cause of these test failures
...
auto CatmullRomSpline::getPose(const double s, const bool fill_pitch) const
  ...
  case 1:
    THROW_SIMULATION_ERROR("Only 1 control point exists for the spline.",
                           "So, the pose cannot be calculated.",
                           "This message is not originally intended to be "
                           "displayed, if you see it, please "
                           "contact the developer of traffic_simulator.");
  case 2:
...
auto CatmullRomSpline::checkConnection() const -> bool {
  ...
  for (size_t i = 0; i < curves_.size(); i++) {
    ...
    if (equals(control_point0, p0) && equals(control_point1, p1)) {
      ...
      THROW_SIMULATION_ERROR(                 // LCOV_EXCL_LINE
          "start point of the curve number ", i,
          " does not match");                 // LCOV_EXCL_LINE
    } else if (!equals(control_point1, p1)) { // LCOV_EXCL_LINE
      THROW_SIMULATION_ERROR(                 // LCOV_EXCL_LINE
          "end point of the curve number ", i,
          " does not match"); // LCOV_EXCL_LINE
    }
  }
  if (curves_.empty()) {
...
auto CatmullRomSpline::equals(const geometry_msgs::msg::Point &p0,
                              const geometry_msgs::msg::Point &p1) const
    -> bool {
