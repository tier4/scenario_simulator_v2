...
TEST(CatmullRomSpline, getCollisionPointIn2DPolygon)
{
  ...
  const auto collision_s2 = spline.getCollisionPointIn2D(points, false, false);
  EXPECT_FALSE(collision_s2);

  const auto collision_s3 = spline.getCollisionPointIn2D(points, true, false);
  EXPECT_FALSE(collision_s3);
}
...
TEST(CatmullRomSpline, getCollisionPointIn2DEmpty)
{
  ...
  const auto collision_s2 = spline.getCollisionPointIn2D(polygon, false, false);
  EXPECT_FALSE(collision_s2);

  const auto collision_s3 = spline.getCollisionPointIn2D(polygon, true, false);
  EXPECT_FALSE(collision_s3);
}
...
TEST(CatmullRomSpline, getRightBoundsPoint)
{
  const math::geometry::CatmullRomSpline spline = makeCurve();
  constexpr double eps = 5e-2;

  const auto point0 = spline.getRightBoundsPoint(2, 1.47895776);
  EXPECT_POINT_NEAR(point0, makePoint(1, 2), eps);

  const auto point1 = spline.getRightBoundsPoint(2, 0);
  EXPECT_POINT_NEAR(point1, makePoint(-0.89442719, 0.44721359), eps);
}

TEST(CatmullRomSpline, getRightBoundsPointOutside)
{
  const math::geometry::CatmullRomSpline spline = makeLine();
  const auto normal_vector = makeVector(-3.0, 1.0);
  const double width = 3;
  const double theta = std::atan2(normal_vector.y, normal_vector.x);

  auto control_point0 = makePoint(3.0, 9.0);
  const auto point0 =
    spline.getRightBoundsPoint(width, std::hypot(control_point0.x, control_point0.y));
  addOffset(control_point0, width / 2.0, theta);
  EXPECT_POINT_NEAR(point0, control_point0, EPS);

  auto control_point1 = makePoint(213.0, 639.0);
  const auto point1 =
    spline.getRightBoundsPoint(width, std::hypot(control_point1.x, control_point1.y));
  addOffset(control_point1, width / 2.0, theta);
  EXPECT_POINT_NEAR(point1, control_point1, EPS);
}

TEST(CatmullRomSpline, getRightBoundsPointNegativeWidth)
{
  const math::geometry::CatmullRomSpline spline = makeCurve();
  constexpr double eps = 5e-2;

  const auto point0 = spline.getRightBoundsPoint(-2, 1.47895776);
  EXPECT_POINT_NEAR(point0, makePoint(1, 0), eps);

  const auto point1 = spline.getRightBoundsPoint(-2, 0);
  EXPECT_POINT_NEAR(point1, makePoint(0.89442719, -0.44721359), eps);
}

TEST(CatmullRomSpline, getLeftBoundsPoint)
{
  const math::geometry::CatmullRomSpline spline = makeCurve();
  constexpr double eps = 5e-2;

  const auto point0 = spline.getLeftBoundsPoint(2, 1.47895776);
  EXPECT_POINT_NEAR(point0, makePoint(1, 0), eps);

  const auto point1 = spline.getLeftBoundsPoint(2, 0);
  EXPECT_POINT_NEAR(point1, makePoint(0.89442719, -0.44721359), eps);
}

TEST(CatmullRomSpline, getLeftBoundsPointOutside)
{
  const math::geometry::CatmullRomSpline spline = makeLine();
  /**
   * @brief The normal vector is inverted because we want to get the left bounds point
   */
  const auto normal_vector = makeVector(3.0, -1.0);
  const double width = 3;
  const double theta = std::atan2(normal_vector.y, normal_vector.x);
  ...
  auto control_point0 = makePoint(3.0, 9.0);
  const auto point0 =
    spline.getLeftBoundsPoint(width, std::hypot(control_point0.x, control_point0.y));
  addOffset(control_point0, width / 2.0, theta);
  EXPECT_POINT_NEAR(point0, control_point0, EPS);

  auto control_point1 = makePoint(213.0, 639.0);
  const auto point1 =
    spline.getLeftBoundsPoint(width, std::hypot(control_point1.x, control_point1.y));
  addOffset(control_point1, width / 2.0, theta);
  EXPECT_POINT_NEAR(point1, control_point1, EPS);
}

TEST(CatmullRomSpline, getLeftBoundsPointNegativeWidth)
{
  const math::geometry::CatmullRomSpline spline = makeCurve();
  constexpr double eps = 5e-2;
...
TEST(CatmullRomSpline, initializationLine)
{
  ...
  geometry_msgs::msg::Point p3 = makePoint(3, 9);
...
TEST(CatmullRomSpline, getSValue1)
{
  ...
  const auto points = {p0, p1, p2, p3};
  const auto spline = math::geometry::CatmullRomSpline(points);
  const auto result = spline.getSValue(makePose(0.1, 0));
  EXPECT_TRUE(result);
  EXPECT_NEAR(result.value(), 0.1, EPS);
  EXPECT_FALSE(spline.getSValue(makePose(10, 0), 3));
}
...
TEST(CatmullRomSpline, getTrajectoryLine)
{
  ...
  const auto trajectory = spline.getTrajectory(0, 3, 1.0);
  ...
  const auto trajectory_reverse = spline.getTrajectory(3, 0, 1.0);
  EXPECT_EQ(trajectory_reverse.size(), static_cast<size_t>(4));
  EXPECT_NEAR(trajectory_reverse[0].x, 3, EPS);
  EXPECT_NEAR(trajectory_reverse[1].x, 2, EPS);
  EXPECT_NEAR(trajectory_reverse[2].x, 1, EPS);
  EXPECT_NEAR(trajectory_reverse[3].x, 0, EPS);
}
...
TEST(CatmullRomSpline, getTrajectoryCurve)
{
  ...
  const auto trajectory = spline.getTrajectory(0, 2.957916, 0.5);
  EXPECT_POINT_NEAR(trajectory[0], makePoint(0, 0), EPS);
  ...
  EXPECT_POINT_NEAR(trajectory[6], makePoint(0, 2), EPS);

  const auto trajectory_offset = spline.getTrajectory(0, 2.957916, 0.5, -1);
...
{
  const math::geometry::CatmullRomSpline spline = makeCurve2();
  const auto trajectory = spline.getTrajectory(0, 1.47895776, 2);
  constexpr double eps = 1e-2;
  EXPECT_EQ(trajectory.size(), static_cast<size_t>(2));
  EXPECT_POINT_NEAR(trajectory[0], makePoint(0, 0), eps);
  EXPECT_POINT_NEAR(trajectory[1], makePoint(1, 1), eps);
}
