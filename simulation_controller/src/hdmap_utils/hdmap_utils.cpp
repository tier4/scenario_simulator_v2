...
namespace hdmap_utils
{
    ...
    boost::optional<double> HdMapUtils::getCollisionPointInLaneCoordinate(int lanelet_id, int crossing_lanelet_id)
    {
        ...
        for (size_t i = 0; i < center_points.size() - 1; ++i)
        {
            ...
            double line_length = std::sqrt(std::pow(p0.x-p1.x,2) + std::pow(p0.y-p1.y,2) + std::pow(p0.z-p1.z,2));
    ...
    boost::optional<std::pair<simulation_controller::math::HermiteCurve,double>> HdMapUtils::getLaneChangeTrajectory(geometry_msgs::msg::Pose from_pose, int to_lanelet_id)
    {
        ...
        for(double to_s=0; to_s<to_length; to_s=to_s+1.0)
        {
            ...
            if(goal_pose)
            {
                double start_to_goal_dist = std::sqrt(std::pow(from_pose.position.x - goal_pose->pose.position.x,2)
    ...
    boost::optional<geometry_msgs::msg::Vector3> HdMapUtils::getTangentVector(int lanelet_id, double s)
    {
        ...
        tangent_vec.y = (resampled_y[1] - resampled_y[0])/diff;
        tangent_vec.z = (resampled_z[1] - resampled_z[0])/diff;
        double vec_size = std::sqrt(tangent_vec.x*tangent_vec.x + tangent_vec.y*tangent_vec.y + tangent_vec.z*tangent_vec.z);
    ...
    boost::optional<double> HdMapUtils::getLongitudinalDistance(int from_lanelet_id, double from_s, int to_lanelet_id, double to_s)
    {
    ...
    std::vector<double> HdMapUtils::calcEuclidDist(const std::vector<double> & x, const std::vector<double> & y, const std::vector<double> & z)
    {
