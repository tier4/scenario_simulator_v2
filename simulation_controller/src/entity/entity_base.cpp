    ...
    namespace entity
    {
        ...
        const EntityStatus EntityBase::getStatus(CoordinateFrameTypes coordinate) const
        {
            ...
            if(coordinate == CoordinateFrameTypes::LANE)
            {
                ...
                {
                    throw SimulationRuntimeError("currentry, projecting world frame into lane frame does not support");
                }
            }
            if(coordinate == CoordinateFrameTypes::WORLD)
            {
                ...
                {
                    auto map_pose = hdmap_utils_ptr_->toMapPose(this->status_->lanelet_id, this->status_->s, this->status_->offset, this->status_->rpy);
                    if(map_pose)
                    {
                        return EntityStatus(this->status_->time, map_pose->pose, this->status_->twist, this->status_->accel);
                    }
                    else
                    {
                ...
                else
                {
                    ...
                    if(ids.size() == 0)
                    {
                        ...
                        if(this->status_->coordinate == CoordinateFrameTypes::LANE)
                        {
                            ...
                            this->status_ = EntityStatus(this->status_->time, following_ids[0], l, this->status_->offset, this->status_->rpy, this->status_->twist, this->status_->accel);
                            return true;
                        }
                        throw SimulationRuntimeError("failed to calculate map pose at the end of the lanelet");
                    }
                    ...
                    this->status_ = EntityStatus(this->status_->time, ids[0], l, this->status_->offset, this->status_->rpy, this->status_->twist, this->status_->accel);
                    return true;
                }
            }
            this->status_ = status;
