...
namespace simulation_controller
{
    namespace entity
    {
        ...
        boost::optional<double> EntityManager::getStandStillDuration(std::string name)
        {
        ...
        boost::optional<double> EntityManager::getLongitudinalDistance(std::string from, std::string to, double max_distance)
        {
        ...
        CoordinateFrameTypes EntityManager::getEntityStatusCoordinate(std::string name)
        {
            auto it = entities_.find(name);
            if(it->second.type() == typeid(VehicleEntity))
            {
        ...
        std::vector<std::string> EntityManager::getVehicleEntityNames()
        {
            ...
            for(auto it = entities_.begin(); it != entities_.end(); it++)
            {
                if(it->second.type() == typeid(VehicleEntity))
                {
                    ret.push_back(boost::any_cast<const VehicleEntity&>(it->second).name);
                }
            }
            return ret;
        }
        
        std::vector<std::string> EntityManager::getEgoEntityNames()
        {
            std::vector<std::string> ret;
            for(auto it = entities_.begin(); it != entities_.end(); it++)
            {
                if(it->second.type() == typeid(EgoEntity))
                {
                    ret.push_back(boost::any_cast<const EgoEntity&>(it->second).name);
                }
            }
            return ret;
        }
        
        boost::optional<VehicleParameters> EntityManager::getVehicleParameters(std::string name)
        {
            auto it = entities_.find(name);
            if (it == entities_.end())
            {
        ...
        bool EntityManager::entityStatusSetted(std::string name)
        {
            auto it = entities_.find(name);
            if (it == entities_.end())
            {
        ...
        void EntityManager::requestAcquirePosition(std::string name, int lanelet_id, double s, double offset)
        {
            auto it = entities_.find(name);
            if (it == entities_.end())
            {
        ...
        std::vector<std::string> EntityManager::getEntityNames()
        {
            std::vector<std::string> ret;
            for(auto it = entities_.begin(); it != entities_.end(); it++)
            {
        ...
        boost::optional<EntityStatus> EntityManager::getEntityStatus(std::string name, CoordinateFrameTypes coordinate)
        {
            auto it = entities_.find(name);
            if (it == entities_.end())
            {
