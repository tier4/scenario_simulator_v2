...
namespace simulation_controller
{
    namespace entity
    {
        EgoEntity::EgoEntity(std::string name, const EntityStatus &initial_state, const pugi::xml_node & xml) 
        : VehicleEntity(name, initial_state , xml) {}
        EgoEntity::EgoEntity(std::string name, const EntityStatus &initial_state, VehicleParameters parameters)
