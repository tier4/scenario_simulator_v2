    ...
    if (target_speed) {
      ...
      for (const auto & status : conflicting_entity_status) {
        for (const auto & lanelet_id : following_lanelets) {
          auto stop_position_s = hdmap_utils_ptr->getCollisionPointInLaneCoordinate(lanelet_id,
              status.lanelet_id);
          if (stop_position_s) {
            auto dist = hdmap_utils_ptr->getLongitudinalDistance(entity_status.lanelet_id,
                entity_status.s,
      ...
      if (dists.size() != 0) {
        ...
        auto dist_to_stop_target = hdmap_utils_ptr->getLongitudinalDistance(
        ...
        if (dist_to_stop_target) {
          double rest_distance = dist_to_stop_target.get() -
            (vehicle_param_ptr->bounding_box.dimensions.length + 5);
