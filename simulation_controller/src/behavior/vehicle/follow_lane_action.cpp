...
namespace entity_behavior
{
    namespace vehicle
    {
        ...
        BT::NodeStatus FollowLaneAction::tick()
        {
            ...
            if(!getInput<std::shared_ptr<simulation_controller::entity::VehicleParameters>>("vehicle_parameters", vehicle_param_ptr))
            {
            ...
            {
                throw BehaviorTreeRuntimeError("failed to get input other_entity_status in FollowLaneAction");
            }
            ...
            {
                throw BehaviorTreeRuntimeError("failed to get input entity_type_list in FollowLaneAction");
            }
            ...
            if(entity_status.coordinate == simulation_controller::entity::CoordinateFrameTypes::WORLD)
            {
                if(target_speed)
                {
                    if(target_speed.get() > vehicle_param_ptr->performance.max_speed)
                    {
                        target_speed = vehicle_param_ptr->performance.max_speed;
                    }
                }
                else
                {
                ...
                simulation_controller::entity::EntityStatus entity_status_updated(current_time + step_time, pose_new, twist_new, accel_new);
            ...
            if(entity_status.coordinate == simulation_controller::entity::CoordinateFrameTypes::LANE)
            {
                for(const auto & each: other_entity_status)
                {
                    ...
                    {
                        auto distance = hdmap_utils_ptr->getLongitudinalDistance(entity_status.lanelet_id, entity_status.s, each.second.lanelet_id, each.second.s);
                        if(distance)
                        {
                            if(distance.get() < 40)
                            {
                ...
                if(target_speed)
                {
                    ...
                    for(const auto & status : other_entity_status)
                    {
                        ...
                        {
                            if(std::count(conflicting_crosswalks.begin(),conflicting_crosswalks.end(),status.second.lanelet_id) >= 1)
                            {
                    ...
                    for(const auto & status : conflicting_entity_status)
                    {
                        ...
                        {
                            auto stop_position_s = hdmap_utils_ptr->getCollisionPointInLaneCoordinate(lanelet_id, status.lanelet_id);
                            if(stop_position_s)
                            {
                                auto dist = hdmap_utils_ptr->getLongitudinalDistance(entity_status.lanelet_id, entity_status.s,
                    ...
                    if(dists.size() != 0)
                    {
                        ...
                        auto dist_to_stop_target = hdmap_utils_ptr->getLongitudinalDistance(entity_status.lanelet_id,entity_status.s,
                            stop_target_status.lanelet_id, stop_target_status.s);
                        if(dist_to_stop_target)
                        {
                            double rest_distance = dist_to_stop_target.get() - (vehicle_param_ptr->bounding_box.dimensions.length + 5);
                ...
                std::vector<geometry_msgs::msg::Point> following_trajectory = hdmap_utils_ptr->clipTrajectoryFromLaneletIds(entity_status.lanelet_id,entity_status.s,following_lanelets,entity_status.twist.linear.x);
                geometry_msgs::msg::Accel accel_new;
                accel_new = entity_status.accel;
