...
public:
  ...
  simulation_controller::entity::EntityStatus getEntityStatus(
    std::string name,
    simulation_controller::entity::CoordinateFrameTypes corrdinate =
    simulation_controller::entity::CoordinateFrameTypes::LANE)
  {
  ...
  bool isInLanelet(std::string name, int lanelet_id)
  {
    ...
    try {
      simulation_controller::entity::EntityStatus status = getEntityStatus(name,
          simulation_controller::entity::CoordinateFrameTypes::LANE);
  ...
  simulation_controller::entity::EntityStatus toStatus(XmlRpc::XmlRpcValue param)
  {
    ...
    if (coordinate == "lane") {
      ...
      simulation_controller::entity::EntityStatus status(time, lanelet_id, s, offset, rpy, twist,
        accel);
      return status;
    }
    if (coordinate == "world") {
