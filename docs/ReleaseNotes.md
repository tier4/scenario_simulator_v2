...
| Legacy parameter distribution            | In xosc generation by `openscenario_utility`, in order to fix the problem that the length of the generated file name is too long, instead of encoding the parameter name and its value in the file name, just change it to a sequential number. | `openscenario_utility`     | [#972](https://github.com/tier4/scenario_simulator_v2/pull/972) | [yamacir-kit](https://github.com/yamacir-kit) |
