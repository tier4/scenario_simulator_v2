...
Velocity trafficSignToVelocity(const std::string & typeString)
{
  ...
  try {
    ...
    throw lanelet::InterpretationError(
            "Unable to interpret the velocity information from " + typeString);
  }
}
}  // namespace

Optional<SpeedLimitInformation> GermanVehicle::speedLimit(
  const RegulatoryElementConstPtrs & regelems) const
{
