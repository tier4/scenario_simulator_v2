...
private:
  ...
  {
    BasicPoint3d delta =
      (utils::toBasicPoint(poly[(after + 1) % poly.size()]) - utils::toBasicPoint(poly[after])) /
      (2 * nNew + 1);
    std::vector<Point3d> points;
    points.reserve(size_t(nNew));
    for (int i = 0; i < nNew; ++i) {
      points.emplace_back(Point3d(++id,
        utils::toBasicPoint(poly[after]) + (2 * double(i) + randomNum_() + 1) * delta));
    }
...
using AllPolygons =
  testing::Types<Polygon2d, Polygon3d, ConstPolygon2d, ConstPolygon3d, ConstHybridPolygon2d,
...
using ThreeDPolygons =
  testing::Types<Polygon3d, ConstPolygon3d, ConstHybridPolygon3d, CompoundPolygon3d,
    CompoundHybridPolygon3d>;
using TwoDPolygons =
  testing::Types<Polygon2d, ConstPolygon2d, ConstHybridPolygon2d, CompoundPolygon2d,
    CompoundHybridPolygon2d>;
using ThreeDAndBasicPolygons = testing::Types<BasicPolygon3d, Polygon3d, ConstPolygon3d,
    ConstHybridPolygon3d,
    CompoundPolygon3d, CompoundHybridPolygon3d>;
using TwoDAndBasicPolygons = testing::Types<BasicPolygon2d, Polygon2d, ConstPolygon2d,
...
using NonHybridPolygons =
  testing::Types<Polygon2d, Polygon3d, ConstPolygon2d, ConstPolygon3d, CompoundPolygon2d,
    CompoundPolygon3d>;
using HybridPolygons =
  testing::Types<ConstHybridPolygon2d, ConstHybridPolygon3d, CompoundHybridPolygon2d,
    CompoundHybridPolygon3d>;
...
TYPED_TEST(TwoDPolygonsTest, convexPartition) {
  ...
  checkPartitionConsistency(BasicPolygon2d(this->simpleStar.basicBegin(),
    this->simpleStar.basicEnd()), t2);
  auto t3 = lanelet::geometry::convexPartition(this->fancyStar);
  EXPECT_EQ(t3.size(), 2ul);
  for (const auto & poly : t3) {
    EXPECT_TRUE(isConvex(poly));
  }
  checkPartitionConsistency(BasicPolygon2d(this->fancyStar.basicBegin(),
    this->fancyStar.basicEnd()), t3);
}
