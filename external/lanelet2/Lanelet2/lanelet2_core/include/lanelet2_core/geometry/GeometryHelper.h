...
std::enable_if_t<std::is_same<InPointT, OutPointT>::value> convertOrAssign(
  const InPointT & pIn,
  OutPointT & pOut)
{
  pOut = pIn;
...
public:
  ...
  auto updateClosestPoint(
    const Point1T & pSeg1, const Point1T & pSeg2, Point2T & pProj,
    CalculationResult d) const
  {
  ...
  inline typename calculation_type<Point, PointOfSegment>::type apply(
    Point const & p, PointOfSegment const & p1,
...
public:
  ...
  static inline return_type apply(
    lanelet::helper::ProjectedPoint<ProjPoint> const &,                               // NOLINT
    T const & value)
  {
    typename lanelet::helper::ProjectedPoint<ProjPoint>::Strategy s;
    return result_from_distance<typename lanelet::helper::ProjectedPoint<ProjPoint>::Strategy, P,
             PS>::apply(s, value);
  }
};
}  // namespace services
