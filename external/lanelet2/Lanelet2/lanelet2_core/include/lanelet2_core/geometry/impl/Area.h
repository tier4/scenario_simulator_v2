...
{
  return overlaps2d(traits::to2D(area.outerBoundPolygon()),
           traits::to2D(otherArea.outerBoundPolygon()));
}
...
template<typename AreaT, typename LaneletT>
IfAr<AreaT, IfLL<LaneletT, bool>> overlaps3d(
...
inline bool leftOf(const ConstLanelet & right, const ConstArea & left)
{
  return utils::anyOf(left.outerBound(), [&right](auto & bound) {
             return bound.invert() == right.leftBound();
           });
}

inline bool rightOf(const ConstLanelet & left, const ConstArea & area)
{
  return leftOf(left.invert(), area);
}
...
inline bool follows(const ConstLanelet & prev, const ConstArea & next)
{
  ...
  return utils::anyOf(outer,
           [p1 = prev.leftBound().back(),
           p2 = prev.rightBound().back()](const ConstLineString3d & ls) {
...
{
  return follows(next.invert(), prev);
}
...
inline bool adjacent(const ConstArea & area1, const ConstArea & area2)
{
  ...
  auto rotatedNext = [ & ls = outer2](auto iter) {
      return iter + 1 == ls.end() ? ls.begin() : iter + 1;
    };
...
inline Optional<ConstLineString3d> determineCommonLinePreceding(
...
inline Optional<ConstLineString3d> determineCommonLineFollowing(
  const ConstArea & ar,
  const ConstLanelet & ll)
{
...
inline Optional<ConstLineString3d> determineCommonLineFollowingOrPreceding(
  const ConstArea & ar,
  const ConstLanelet & ll)
{
  return utils::findIf(ar.outerBound(), [p1 = ll.leftBound().front(), p2 = ll.rightBound().front(),
           p3 = ll.leftBound().back(), p4 = ll.rightBound().back()](auto & boundLs) {
             return (boundLs.back() == p2 && boundLs.front() == p1) ||
             (boundLs.back() == p3 && boundLs.front() == p4);
           });
}

inline Optional<ConstLineString3d> determineCommonLineSideways(
  const ConstLanelet & ll,
  const ConstArea & ar)
{
...
inline Optional<ConstLineString3d> determineCommonLineSideways(
  ...
  const ConstLanelet & ll)
{
  return utils::findIf(ar.outerBound(), [lb = ll.leftBound(), rb = ll.rightBound()](
             auto & boundLs) {
...
inline Optional<ConstLineString3d> determineCommonLine(
  const ConstArea & ar,
  const ConstLanelet & ll)
{
...
inline Optional<ConstLineString3d> determineCommonLineLeft(
  const ConstLanelet & right,
...
inline Optional<ConstLineString3d> determineCommonLineRight(
  const ConstLanelet & left,
  const ConstArea & right)
{
  return utils::findIf(right.outerBound(), [&left](auto & bound) {
             return bound == left.rightBound();
           });
}
