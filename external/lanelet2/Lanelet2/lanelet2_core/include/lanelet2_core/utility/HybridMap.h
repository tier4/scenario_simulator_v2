...
std::vector<Iterator> copyIterators(
  const std::vector<Iterator> & oldV, const Map & oldM,
  Map & newM)
{
...
public:
  ...
  : m_(std::move(rhs.m_)), v_{std::move(rhs.v_)}
  {
  ...
  : m_{rhs.m_}, v_{detail::copyIterators(rhs.v_, rhs.m_, m_)} {}
