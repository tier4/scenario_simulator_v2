...
public:
   ...
   */
  explicit ConstPoint2d(
    Id id = InvalId, double x = 0., double y = 0., double z = 0.,
...
public:
  ...
  explicit ConstPoint3d(const ConstPoint2d & other)
  : ConstPoint2d(other) {}
...
public:
  ...
  explicit Point3d(const Point2d & other)
  : Point2d(other) {}
...
{
  return stream << "[id: " << obj.id() << " x: " << obj.x() << " y: " << obj.y() << " z: " <<
         obj.z() << "]";
}
