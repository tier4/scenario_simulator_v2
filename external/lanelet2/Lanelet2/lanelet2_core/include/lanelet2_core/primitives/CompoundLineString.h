...
struct SelectCompoundLsIterator
{
  ...
  using Iterator = internal::TransformIterator<internal::ReverseAndForwardIterator<BaseIterator>,
      const PointT>;
};
...
public:
  ...
  explicit CompoundLineStringData(ConstLineStrings3d lineStrings)
  : ls_{std::move(lineStrings)} {}
...
public:
  ...
  {
    return inverted() ?
           utils::invert(utils::transform(data_->lineStrings(), [](const auto & ls) {
               return ls.invert();
             })) :
           data_->lineStrings();
  }
...
public:
  ...
  {
    return CompoundLineString2d{constData(), !inverted()};
  }
};
...
public:
  ...
  {
    return CompoundLineString3d{constData(), !inverted()};
  }
};
...
public:
  ...
  : CompoundHybridLineString2d(utils::concatenate(other,
      [](auto & cls) {return cls.lineStrings();})) {}
  CompoundHybridLineString2d() = default;  // Not inherited from base class

  //! create a new, inverted linestring from this one
  CompoundHybridLineString2d invert() const noexcept
  {
    return CompoundHybridLineString2d{constData(), !inverted()};
  }
};
...
public:
  ...
  : CompoundHybridLineString3d(utils::concatenate(other,
      [](auto & cls) {return cls.lineStrings();})) {}
  ...
  {
    return CompoundHybridLineString3d{constData(), !inverted()};
  }
};
...
bool operator==(
  const CompoundLineStringImpl<Point1T> & lhs,
