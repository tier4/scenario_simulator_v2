...
{
  return units::KmHQuantity(self.speedLimit).value();
}
...
{
  return units::MPSQuantity(self.speedLimit).value();
}
...
{
  return self.isOneWay(llt);
}
bool hasDynamicRulesWrapper(const TrafficRules & self, const ConstLanelet & llt)
{
  return self.hasDynamicRules(llt);
}

TrafficRulesPtr createTrafficRulesWrapper(
...
BOOST_PYTHON_MODULE(PYTHON_API_MODULE_NAME) {  // NOLINT
  ...
  class_<SpeedLimitInformation>("SpeedLimitInformation",
  ...
  .def("canPass", canPassWrapper<ConstLanelet>,
    "Returns whether it is allowed to pass/drive on this lanelet")
  .def("canPass", canPassWrapper<ConstArea>,
