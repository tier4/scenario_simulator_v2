...
std::shared_ptr<LaneletMap> loadProjectorWrapper(
...
void writeProjectorWrapper(
  const std::string & filename, const LaneletMap & map,
  const Projector & projector)
{
  write(filename, map, projector);
}

ErrorMessages writeWithErrorWrapper(
  const std::string & filename, const LaneletMap & map,
  const Projector & projector)
{
  ErrorMessages errs;
...
BOOST_PYTHON_MODULE(PYTHON_API_MODULE_NAME) {  // NOLINT
  ...
  def("write", writeProjectorWrapper,
    (arg("filename"), arg("map"), arg("projector") = DefaultProjector()),
  ...
  def("write", writeWrapper, (arg("filename"), arg("map"), arg("origin")),
    "Writes the map to a file. The extension determines which format will "
    "be used (usually .osm)");
  def("writeRobust", writeWithErrorWrapper,
    (arg("filename"), arg("map"), arg("projector") = DefaultProjector()),
