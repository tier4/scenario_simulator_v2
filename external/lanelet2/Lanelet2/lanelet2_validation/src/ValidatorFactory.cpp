...
std::vector<std::string> matchSingleRegex(
...
std::vector<std::string> matchValidators(const Regexes & regexes, const RegistryT & registry)
{
  ...
  auto matches = utils::concatenate(regexes, [&toMatch](auto & regex) {
...
ValidatorsWithName<ValidatorT> createValidators(
  const Regexes & regexes, const std::map<std::string,
  ValidatorFactory::CreationFcn<ValidatorT>> & registry)
{
...
ValidatorsWithName<TrafficRuleValidator> ValidatorFactory::createTrafficRuleValidators(
  const Regexes & regexes)
{
  return createValidators(regexes, trafficRuleValidatorRegistry_);
}

ValidatorsWithName<RoutingGraphValidator> ValidatorFactory::createRoutingGraphValidators(
  const Regexes & regexes)
{
...
void ValidatorFactory::registerMapValidator(
...
void ValidatorFactory::registerTrafficRuleValidator(
  const std::string & name,
...
void ValidatorFactory::registerRoutingGraphValidator(
  const std::string & name,
