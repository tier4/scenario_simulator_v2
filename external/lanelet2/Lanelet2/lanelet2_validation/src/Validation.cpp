...
void runMapValidators(
  std::vector<DetectedIssues> & issues, const Regexes & regexes,
...
void runRuleValidators(
  std::vector<DetectedIssues> & issues, const Regexes & regexes, const LaneletMap & map,
  const std::vector<traffic_rules::TrafficRulesUPtr> & rules)
{
  ...
  append(issues, runValidators(validators, [&map, &rules](auto & validator) {
      return validator(map, rules);
    }));
}

void runRoutingGraphValidators(
  std::vector<DetectedIssues> & issues, const Regexes & regexes, LaneletMap & map,
  const std::vector<traffic_rules::TrafficRulesUPtr> & rules)
{
...
IssueReport buildReport(std::vector<DetectedIssues> issues)
{
  ...
  for (auto & issue : issues) {
    auto buildReports = [ & check = issue.checkName](auto & issue) {
        return issue.buildReport() + " [" + check + "]";
      };
    ...
    if (!warningsFromCheck.empty()) {
      report.warnings.insert(report.warnings.end(), warningsFromCheck.begin(),
        warningsFromCheck.end());
    }
  }
  return report;
}
...
std::vector<DetectedIssues> validateMap(
