...
public:
  ...
  virtual double getCostSucceeding(
  ...
  virtual double getCostLaneChange(
...
public:
  ...
  : laneChangeCost_{laneChangeCost}, minChangeLength_{minLaneChangeLength}
  {
    if (laneChangeCost_ < 0.0) {
      throw InvalidInputError("Lane change cost must be positive, but it is " +
              std::to_string(laneChangeCost_));
    }
  }
    ...
    const ConstLanelets & /*to*/) const noexcept override
  {
    ...
    return totalLength >=
           minChangeLength_ ? laneChangeCost_ : std::numeric_limits<double>::infinity();
  }
...
public:
    ...
    const ConstLanelets & /*to*/) const noexcept override
  {
    ...
    auto changeTime =
      std::accumulate(from.begin(), from.end(), 0., [this, &trafficRules](double acc,
        auto & llt) {
...
{
  return RoutingCostPtrs{std::make_shared<RoutingCostDistance>(10.),
    std::make_shared<RoutingCostTravelTime>(5.)};
}
}  // namespace routing
}  // namespace lanelet
