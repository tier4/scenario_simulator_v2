...
public:
  ...
  explicit RoutingGraphContainer(std::vector<RoutingGraphConstPtr> routingGraphs)
  : graphs_{std::move(routingGraphs)} {}
  ...
  ConstLanelets conflictingInGraph(
    const ConstLanelet & lanelet, size_t routingGraphId,
  ...
  ConflictingInGraphs conflictingInGraphs(
   ...
   *  @throws InvalidInputError if the routingGraphId is too high */
  ConstLanelets conflictingOfRouteInGraph(
    const Route * route, size_t routingGraphId,
    double participantHeight = .0) const
  {
  ...
  ConflictingInGraphs conflictingOfRouteInGraphs(
    const Route * route,
    double participantHeight = .0) const
  {
    ConflictingInGraphs result;
    for (size_t it = 0; it < graphs_.size(); it++) {
      result.emplace_back(std::make_pair(it, conflictingOfRouteInGraph(route, it,
        participantHeight)));
    }
    return result;
  }
