...
public:
  ...
  explicit OnRouteFilter(const RouteLanelets & onRoute)
  : onRoute_{&onRoute} {}
...
public:
  ...
  explicit OnlyDrivableEdgesFilter(const OriginalGraph & originalGraph)
  ...
  bool operator()(FilteredRoutingGraph::edge_descriptor e) const
  {
    ...
    return (type & (RelationType::Successor | RelationType::Left | RelationType::Right)) !=
           RelationType::None;
  }
...
public:
  ...
  explicit OnlyConflictingFilter(const OriginalGraph & originalGraph)
  : originalGraph_{&originalGraph} {}
  bool operator()(FilteredRoutingGraph::edge_descriptor e) const
  {
    auto type = (*originalGraph_)[e].relation;
...
public:
  ...
  explicit NoConflictingFilter(const OriginalGraph & originalGraph)
  : originalGraph_{&originalGraph} {}
  bool operator()(FilteredRoutingGraph::edge_descriptor e) const
  {
    auto type = (*originalGraph_)[e].relation;
...
public:
  ...
  bool operator()(LaneletVertexId vertexId) const
  {
    ...
    auto connectedToRoute =
      std::any_of(outEdges.first, outEdges.second, [&](OriginalGraph::edge_descriptor e) {
...
public:
  ...
  bool operator()(LaneletVertexId vertexId) const
  {
    ...
    std::for_each(outEdges.first, outEdges.second, [&](auto edge) {
        if (onRoute_->find(boost::target(edge,
        *g_)) == onRoute_->end())
        {
          return;
        }
        ...
        if ((type & (neighbourTypes)) != RelationType::None) {
          auto outEdge = boost::edge(boost::target(edge, *g_), boost::source(edge,
          *g_), *g_);
...
public:
  ...
  explicit OnRouteAndConflictFilter(
    const RouteLanelets & onRoute, const std::vector<LaneletVertexId> & conflictWith,
  ...
  bool operator()(LaneletVertexId vertexId) const
  {
    auto isOk = anySidewayNeighbourIs(vertexId, *g_, [&](auto v) {
          ...
          auto isConflicting = [&](
            OriginalGraph::edge_descriptor e) {
            auto conflictTypes = RelationType::Conflicting | RelationType::AdjacentLeft | RelationType::AdjacentRight;
            auto type = (*g_)[e].relation;
            return (type & conflictTypes) != RelationType::None &&
            has(*conflictWith_, boost::target(e, *g_));
          };
          return v == start_ || v == end_ ||
          std::any_of(outEdges.first, outEdges.second, isConflicting);
        });
    return isOk;
  }
...
public:
  ...
  explicit ConnectedPathIterator(const GraphT & g)
  : g_{g} {}
  ...
  bool hasPathFromTo(LaneletVertexId from, LaneletVertexId to)
  {
    ...
    auto destinationReached = [&](const auto & path) {
        return std::any_of(std::make_reverse_iterator(path.end()),
                 std::make_reverse_iterator(path.begin()),
                 [&](auto p) {return p == to;});
      };
    try {
...
using NextToRouteGraph = boost::filtered_graph<OriginalGraph, OnlyDrivableEdgesFilter,
    NextToRouteFilter>;
using ConflictOrAdjacentToRouteGraph =
  boost::filtered_graph<OriginalGraph, OnlyDrivableEdgesFilter, ConflictingSectionFilter>;
using ConflictsWithPathGraph = boost::filtered_graph<OriginalGraph, NoConflictingFilter,
    OnRouteAndConflictFilter>;
