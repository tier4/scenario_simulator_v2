...
public:
  ...
  explicit VisitedLaneletGraph(const RouteLanelets & llts)
  : routeLanelets_{&llts} {}
...
private:
  void remove(VisitedCounters::value_type & v, const NextToRouteGraph & g)
  {
    ...
    std::for_each(inEdges.first, inEdges.second, [&](NextToRouteGraph::edge_descriptor e) {
        ...
        if ((relation & (RelationType::Successor | RelationType::Left | RelationType::Right)) ==
        RelationType::None)
        {
          return;
        }
  ...
  void forEachVertexOutOfRoute(const VisitedLaneletGraph & visited, Func && f)
  {
    ...
    auto testIfPathIsPermitted = [&](auto & path) {
        ...
        if (this->neverConflictsWithRoute(path) || this->alwaysConflictsWithRoute(path)) {
          for (auto & vertex : path) {
            auto iter = std::find(newConflictingVertices_.begin(),
                newConflictingVertices_.end(), vertex);
    ...
    for (auto newVertex : newConflictingVertices_) {
      ...
      auto connectsToRoute = [&](auto e) {
          constexpr auto AllowedRelation = RelationType::Successor | RelationType::Left |
            RelationType::Right;
...
private:
  bool neverConflictsWithRoute(const LaneletVertexIds & path)
  {
    ...
    auto isAdjacentToRoute = [&](LaneletVertexId v) {
        ...
        constexpr auto Adjacent =
          RelationType::Left | RelationType::Right | RelationType::AdjacentLeft |
          RelationType::AdjacentRight;
  ...
  bool alwaysConflictsWithRoute(const LaneletVertexIds & path)
  {
    ...
    auto fromE = std::find_if(before.first, before.second, [&](auto e) {
          return has(*llts_, boost::source(e, g_));
        });
    auto toE = std::find_if(after.first, after.second, [&](auto e) {
          return has(*llts_, boost::target(e, g_));
        });
...
{
public:
  explicit NeighbouringGraphVisitor(VisitedLaneletGraph & counter)
  : counter_{&counter} {}
  ...
  {
    assert(vCurr_ == boost::source(e,
      g) && "Breadth first search seems to iterate in a weird manner");
...
private:
  ...
  static LaneletVertexId getOnePredecessor(LaneletVertexId v, const OnRouteGraph & g)
  {
    auto inEdges = boost::in_edges(v, g);
    auto e =
      *std::find_if(inEdges.first, inEdges.second, [&](auto e) {
          return g[e].relation == RelationType::Successor;
        });
...
public:
  ...
  bool addAdjacentLaneletsToRoute()
  {
    ...
    bool progress = false;
    laneletVisitor_.forAllValidLanelets([&](auto lltId) {
        progress |= laneletsOnRoute_.insert(lltId.first).second;
      });
    return progress;
  }
  ...
  Optional<Route> finalizeRoute(const OriginalGraph & theGraph, const LaneletPath & thePath)
  {
    ...
    auto map = utils::createConstSubmap(utils::transform(g.vertex_set(), [&g](auto v) {
          return g[v].lanelet;
        }), {});
...
Optional<Route> RouteBuilder::getRouteFromShortestPath(
  const LaneletPath & path, bool withLaneChanges,
  RoutingCostId costId)
{
  // translate the lanelets to graph ids
  auto vertexIds = utils::transform(path, [&](auto llt) {
        return LaneletVertexId{*graph_.getVertex(llt)};
      });
  // decide which graph we are going to use
  OriginalGraph originalGraph{graph_.get(),
    OriginalGraphFilter{graph_.get(), withLaneChanges, costId}};

  // get the container for all the things
  RouteUnderConstruction routeUnderConstruction{vertexIds, originalGraph};

  //! @todo deleteme
  auto ids = utils::transform(graph_.get().m_vertices, [&](auto & v) {
        return v.m_property.laneletOrArea.id();
      });
