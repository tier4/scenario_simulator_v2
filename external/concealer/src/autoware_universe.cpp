...
auto operator<<(std::ostream & out, const BehaviorType & behavior) -> std::ostream &
{
#define CASE(IDENTIFIER)     \
  ...
  switch (behavior.data) {
    ...
    default:
      throw common::Error(
        "Unsupported MrmState::behavior, number : ", static_cast<int>(behavior.data));
  }
...
auto operator>>(std::istream & is, BehaviorType & behavior) -> std::istream &
{
#define BEHAVIOR(IDENTIFIER) {#IDENTIFIER, MrmState::IDENTIFIER}

  std::unordered_map<std::string, std::uint8_t> behavior_dictionary{
    BEHAVIOR(COMFORTABLE_STOP),
...
#undef BEHAVIOR
  ...
  if (auto iter = behavior_dictionary.find(behavior_string); iter != behavior_dictionary.end()) {
    behavior.data = iter->second;
  } else {
    throw common::Error("Unsupported MrmState::behavior : ", behavior_string.c_str());
  }
...
// for MrmState::state
auto operator<<(std::ostream & out, const StateType & state) -> std::ostream &
{
#define CASE(IDENTIFIER)     \
  case MrmState::IDENTIFIER: \
    out << #IDENTIFIER;      \
  ...
  switch (state.data) {
    ...
    default:
      throw common::Error("Unsupported MrmState::state, number : ", static_cast<int>(state.data));
  }
...
auto operator>>(std::istream & is, StateType & state) -> std::istream &
{
...
#undef STATE
  ...
  if (auto iter = state_dictionary.find(state_string); iter != state_dictionary.end()) {
    state.data = iter->second;
  } else {
    throw common::Error("Unsupported MrmState::state, number : ", state_string.c_str());
  }
