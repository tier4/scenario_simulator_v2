...
template <typename T>
auto MinimumRiskManeuverMerger::extractStateName(const T & msg) const -> std::string
{
...
template auto
MinimumRiskManeuverMerger::extractStateName<autoware_auto_system_msgs::msg::EmergencyState>(
  const autoware_auto_system_msgs::msg::EmergencyState &) const -> std::string;
#ifdef USE_ADAPI_V1_MSG
template auto MinimumRiskManeuverMerger::extractStateName<autoware_adapi_v1_msgs::msg::MrmState>(
  const autoware_adapi_v1_msgs::msg::MrmState &) const -> std::string;
#endif

template <typename T>
auto MinimumRiskManeuverMerger::extractBehaviorName(const T & msg) const -> std::string
{
  using RAW_T = std::decay_t<T>;
#define CASE(IDENTIFIER) \
  case T::IDENTIFIER:    \
...
template auto
MinimumRiskManeuverMerger::extractBehaviorName<autoware_auto_system_msgs::msg::EmergencyState>(
  const autoware_auto_system_msgs::msg::EmergencyState &) const -> std::string;
#ifdef USE_ADAPI_V1_MSG
template auto MinimumRiskManeuverMerger::extractBehaviorName<autoware_adapi_v1_msgs::msg::MrmState>(
  const autoware_adapi_v1_msgs::msg::MrmState &) const -> std::string;
#endif
