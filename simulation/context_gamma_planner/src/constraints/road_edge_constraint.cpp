...
{
using namespace traffic_simulator::lanelet_wrapper::lanelet_map;
...
: ConstraintBase(hdmap_utils, LANE_CONSTRAINT, {[&]() {
                   switch (side) {
                     case Side::RIGHT:
                       return rightBound(lanelet_id);
                     case Side::LEFT:
                       return leftBound(lanelet_id);
                     case Side::FRONT: {
                       context_gamma_planner::constraints::Polygon front_bound;
                       front_bound.push_back(rightBound(lanelet_id).back());
                       front_bound.push_back(leftBound(lanelet_id).back());
                       return front_bound;
                     }
                     case Side::BACK: {
                       context_gamma_planner::constraints::Polygon back_bound;
                       back_bound.push_back(rightBound(lanelet_id).front());
                       back_bound.push_back(leftBound(lanelet_id).front());
                       return back_bound;
                     }
                   }
