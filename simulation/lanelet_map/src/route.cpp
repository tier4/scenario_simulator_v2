  ...
  const bool include_self) -> lanelet::Ids
{
  ...
  if (!found) {
    throw std::runtime_error("lanelet id does not match");
  }
  if (total_distance > distance) {
    return ids;
  }
...
auto getSpeedLimit(const lanelet::Ids & lanelet_ids) -> double
{
  ...
  if (lanelet_ids.empty()) {
    throw std::runtime_error("size of the vector lanelet ids should be more than 1");
  }
