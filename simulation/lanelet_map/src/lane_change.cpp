...
#include <lanelet_map/lane_change.hpp>
#include <lanelet_map/other.hpp>
#include <lanelet_map/pose.hpp>
      ...
      if (next_ids.empty()) {
        ...
        if (next_ids.empty()) {
          throw std::runtime_error(
            "failed to calculate along pose (id,s) = (" + std::to_string(from_pose.lanelet_id) +
            "," + std::to_string(from_pose.s + along) +
            "), next lanelet of id = " + std::to_string(along_pose.lanelet_id) + "is empty.");
        }
      }
  ...
  -> std::optional<std::pair<math::geometry::HermiteCurve, double>>
{
  ...
  const auto left_point =
    pose::toMapPose(pose::constructLaneletPose(
  ...
  const auto to_pose = pose::constructLaneletPose(
  ...
  -> std::optional<std::pair<math::geometry::HermiteCurve, double>>
{
  ...
  for (double to_s = 0; to_s < to_length; to_s = to_s + 1.0) {
    auto goal_pose = pose::toMapPose(
      pose::constructLaneletPose(lane_change_parameter.target.lanelet_id, to_s, 0.0));
    if (
  ...
  switch (trajectory_shape) {
    case TrajectoryShape::CUBIC:
      ...
      if (getTangentVector(to_pose.lanelet_id, to_pose.s)) {
        ...
        throw std::runtime_error(
          "Failed to calculate tangent vector at lanelet_id : " +
          std::to_string(to_pose.lanelet_id) + " s : " + std::to_string(to_pose.s));
      }
