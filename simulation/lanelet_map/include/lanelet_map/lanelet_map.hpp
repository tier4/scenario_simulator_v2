    ...
    -> decltype(auto)
  {
    if (!exists(from, to, allow_lane_change)) {
      throw std::runtime_error(
        "route from : " + std::to_string(from) + " to : " + std::to_string(to) +
        (allow_lane_change ? " with" : " without") +
...
public:
  ...
  auto getCenterPoints(lanelet::Id lanelet_id) -> decltype(auto)
  {
    if (!exists(lanelet_id)) {
      throw std::runtime_error(
        "center point of : " + std::to_string(lanelet_id) + " does not exists on route cache.");
    }
    std::lock_guard<std::mutex> lock(mutex_);
...
public:
  ...
  auto getLength(lanelet::Id lanelet_id)
  {
    if (!exists(lanelet_id)) {
      throw std::runtime_error(
        "length of : " + std::to_string(lanelet_id) + " does not exists on route cache.");
    }
    std::lock_guard<std::mutex> lock(mutex_);
