...
TEST(Conversion, ConvertInitializeResponse)
{
  ...
  std::string description = xml[xmlrpc_interface::key_description];
...
TEST(Conversion, ConvertInitializeRequest)
{
  ...
  EXPECT_DOUBLE_EQ(req.step_time(), xml[xmlrpc_interface::key_step_time]);
  EXPECT_DOUBLE_EQ(req.realtime_factor(), xml[xmlrpc_interface::key_realtime_factor]);
