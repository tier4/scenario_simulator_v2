...
StopAtStopLineAction::StopAtStopLineAction(const std::string &name,
...
const traffic_simulator_msgs::msg::WaypointsArray
StopAtStopLineAction::calculateWaypoints() {
...
std::optional<double>
StopAtStopLineAction::calculateTargetSpeed(double current_velocity) {
  if (!distance_to_stopline_) {
    return std::nullopt;
  }
  /**
  ...
  double rest_distance =
      ...
      (vehicle_parameters.bounding_box.dimensions.x * 0.5 + 1.0);
  if (rest_distance <
...
BT::NodeStatus StopAtStopLineAction::tick() {
  ...
  if (std::fabs(canonicalized_entity_status->getTwist().linear.x) < 0.001) {
    if (distance_to_stopline_) {
      if (distance_to_stopline_.value() <=
