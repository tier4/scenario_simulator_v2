...
void VehicleBehaviorTree::configure(const rclcpp::Logger &logger) {
  factory_.registerNodeType<vehicle::follow_lane_sequence::FollowLaneAction>(
      "FollowLane");
  factory_
      .registerNodeType<vehicle::follow_lane_sequence::FollowFrontEntityAction>(
          "FollowFrontEntity");
  factory_.registerNodeType<
      vehicle::follow_lane_sequence::StopAtCrossingEntityAction>(
      "StopAtCrossingEntity");
  factory_
      .registerNodeType<vehicle::follow_lane_sequence::StopAtStopLineAction>(
          "StopAtStopLine");
  factory_.registerNodeType<
      vehicle::follow_lane_sequence::StopAtTrafficLightAction>(
      "StopAtTrafficLight");
  factory_.registerNodeType<vehicle::follow_lane_sequence::YieldAction>(
      "Yield");
  factory_.registerNodeType<vehicle::follow_lane_sequence::MoveBackwardAction>(
      "MoveBackward");
  factory_.registerNodeType<vehicle::FollowPolylineTrajectoryAction>(
  ...
  logging_event_ptr_ = std::make_unique<behavior_tree_plugin::LoggingEvent>(
      tree_.rootNode(), logger);

  reset_request_event_ptr_ =
      std::make_unique<behavior_tree_plugin::ResetRequestEvent>(
...
auto VehicleBehaviorTree::createBehaviorTree(const std::string &format_path)
    -> BT::Tree {
  ...
  public:
    explicit XMLTreeWalker(const BT::TreeNodeManifest &manifest)
        : manifest_(manifest) {}
    ...
    bool for_each(pugi::xml_node &node) final {
      if (node.name() == manifest_.registration_ID) {
        for (const auto &[port, info] : manifest_.ports) {
          node.append_attribute(port.c_str()) =
...
auto VehicleBehaviorTree::getBehaviorParameter()
    -> traffic_simulator_msgs::msg::BehaviorParameter {
  return tree_.rootBlackboard()
      ->get<traffic_simulator_msgs::msg::BehaviorParameter>(
...
auto VehicleBehaviorTree::setBehaviorParameter(
  ...
  auto clamp = [&](const auto &behavior_parameter) {
    ...
    result.dynamic_constraints.max_acceleration =
        std::clamp(result.dynamic_constraints.max_acceleration, 0.0,
                   vehicle_parameters.performance.max_acceleration);

    result.dynamic_constraints.max_acceleration_rate =
        std::clamp(result.dynamic_constraints.max_acceleration_rate, 0.0,
                   vehicle_parameters.performance.max_acceleration_rate);

    result.dynamic_constraints.max_deceleration =
        std::clamp(result.dynamic_constraints.max_deceleration, 0.0,
                   vehicle_parameters.performance.max_deceleration);

    result.dynamic_constraints.max_deceleration_rate =
        std::clamp(result.dynamic_constraints.max_deceleration_rate, 0.0,
                   vehicle_parameters.performance.max_deceleration_rate);

    result.dynamic_constraints.max_speed =
        std::clamp(result.dynamic_constraints.max_speed, 0.0,
...
const std::string &VehicleBehaviorTree::getCurrentAction() const {
  std::cout << "VehicleBehaviorTree::getCurrentAction(): "
            << logging_event_ptr_->getCurrentAction() << std::endl;
  return logging_event_ptr_->getCurrentAction();
}

auto VehicleBehaviorTree::update(const double current_time,
...
auto VehicleBehaviorTree::tickOnce(const double current_time,
...
PLUGINLIB_EXPORT_CLASS(entity_behavior::VehicleBehaviorTree,
