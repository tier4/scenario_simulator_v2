...
BT::PortsList operator+(const BT::PortsList &ports_1,
                        const BT::PortsList &ports_2) {
...
auto ActionNode::getBlackBoardValues() -> void {
  ...
  if (!getInput<std::shared_ptr<hdmap_utils::HdMapUtils>>("hdmap_utils",
                                                          hdmap_utils)) {
  ...
  if (!getInput<std::shared_ptr<traffic_simulator::CanonicalizedEntityStatus>>(
    ...
    THROW_SIMULATION_ERROR(
        "failed to get input canonicalized_entity_status in ActionNode");
  }
  ...
  if (!getInput<double>(
    ...
    THROW_SIMULATION_ERROR(
        "failed to get input matching_distance_for_lanelet_pose_calculation in "
        "ActionNode");
  }

  if (!getInput<EntityStatusDict>("other_entity_status", other_entity_status)) {
    THROW_SIMULATION_ERROR(
        "failed to get input other_entity_status in ActionNode");
  }
...
auto ActionNode::getHorizon() const -> double {
  return std::clamp(canonicalized_entity_status->getTwist().linear.x * 5.0,
                    20.0, 50.0);
}
...
auto ActionNode::setCanonicalizedEntityStatus(
    const traffic_simulator::EntityStatus &entity_status) -> void {
...
auto ActionNode::getOtherEntityStatus(lanelet::Id lanelet_id) const
  ...
  for (const auto &status : other_entity_status) {
    if (status.second.laneMatchingSucceed() &&
...
auto ActionNode::getYieldStopDistance(
    const lanelet::Ids &following_lanelets) const -> std::optional<double> {
  ...
  for (const auto &lanelet : following_lanelets) {
    ...
    for (const auto right_of_way_id : right_of_way_ids) {
      ...
      if (!other_status.empty() &&
          canonicalized_entity_status->laneMatchingSucceed()) {
        ...
        const auto distance_forward = hdmap_utils->getLongitudinalDistance(
            lanelet_pose,
            traffic_simulator::helper::constructLaneletPose(lanelet, 0));
...
auto ActionNode::getRightOfWayEntities(const lanelet::Ids &following_lanelets)
    const -> std::vector<traffic_simulator::CanonicalizedEntityStatus> {
  auto is_the_same_right_of_way = [&](const std::int64_t &lanelet_id,
                                      const std::int64_t &following_lanelet) {
    const auto right_of_way_lanelet_ids =
        hdmap_utils->getRightOfWayLaneletIds(lanelet_id);
    const auto the_same_right_of_way_it =
        std::find(right_of_way_lanelet_ids.begin(),
                  right_of_way_lanelet_ids.end(), following_lanelet);
    return the_same_right_of_way_it != std::end(right_of_way_lanelet_ids);
  };

  std::vector<traffic_simulator::CanonicalizedEntityStatus> ret;
  const auto lanelet_ids_list =
      hdmap_utils->getRightOfWayLaneletIds(following_lanelets);
  for (const auto &status : other_entity_status) {
    for (const auto &following_lanelet : following_lanelets) {
      for (const lanelet::Id &lanelet_id :
           lanelet_ids_list.at(following_lanelet)) {
        if (status.second.laneMatchingSucceed() &&
            traffic_simulator::isSameLaneletId(status.second, lanelet_id) &&
...
auto ActionNode::getRightOfWayEntities() const
  ...
  const auto lanelet_ids = hdmap_utils->getRightOfWayLaneletIds(
      canonicalized_entity_status->getLaneletId());
...
auto ActionNode::getDistanceToTrafficLightStopLine(
  ...
  if (const auto traffic_light_ids =
          hdmap_utils->getTrafficLightIdsOnPath(route_lanelets);
...
auto ActionNode::getFrontEntityName(
    const math::geometry::CatmullRomSplineInterface &spline) const
  ...
  for (const auto &each : other_entity_status) {
    ...
    /**
     * @note hard-coded parameter, if the Yaw value of RPY is in ~1.5708
     * -> 1.5708, entity is a candidate of front entity.
     */
    if (std::fabs(math::geometry::convertQuaternionToEulerAngle(quat).z) <=
  ...
  if (entities.size() != distances.size()) {
    THROW_SIMULATION_ERROR(
        "size of entities and distances vector does not match.");
  }
  if (distances.empty()) {
    return std::nullopt;
  }
...
auto ActionNode::getEntityStatus(const std::string &target_name) const
  ...
  if (auto it = other_entity_status.find(target_name);
      it != other_entity_status.end()) {
    return it->second;
  } else {
...
auto ActionNode::getDistanceToTargetEntityPolygon(
    const math::geometry::CatmullRomSplineInterface &spline,
    const std::string target_name, double width_extension_right,
    double width_extension_left, double length_extension_front,
    double length_extension_rear) const -> std::optional<double> {
  return getDistanceToTargetEntityPolygon(
      spline, getEntityStatus(target_name), width_extension_right,
...
auto ActionNode::getDistanceToTargetEntityPolygon(
    ...
    double width_extension_right, double width_extension_left,
    double length_extension_front,
    double length_extension_rear) const -> std::optional<double> {
  if (isOtherEntityAtConsideredAltitude(status)) {
    const auto polygon = math::geometry::transformPoints(
        status.getMapPose(), math::geometry::getPointsFromBbox(
                                 status.getBoundingBox(), width_extension_right,
                                 width_extension_left, length_extension_front,
...
auto ActionNode::isOtherEntityAtConsideredAltitude(
    const traffic_simulator::CanonicalizedEntityStatus &entity_status) const
    -> bool {
  if (canonicalized_entity_status->laneMatchingSucceed() &&
...
auto ActionNode::getDistanceToConflictingEntity(
    ...
    const math::geometry::CatmullRomSplineInterface &spline) const
    -> std::optional<double> {
  auto crosswalk_entity_status =
      getConflictingEntityStatusOnCrossWalk(route_lanelets);
...
auto ActionNode::getConflictingEntityStatusOnCrossWalk(
    ...
    -> std::vector<traffic_simulator::CanonicalizedEntityStatus> {
  std::vector<traffic_simulator::CanonicalizedEntityStatus>
      conflicting_entity_status;
  auto conflicting_crosswalks =
      hdmap_utils->getConflictingCrosswalkIds(route_lanelets);
  for (const auto &status : other_entity_status) {
    if (status.second.laneMatchingSucceed() &&
        std::count(conflicting_crosswalks.begin(), conflicting_crosswalks.end(),
...
auto ActionNode::getConflictingEntityStatusOnLane(
    const lanelet::Ids &route_lanelets) const
    -> std::vector<traffic_simulator::CanonicalizedEntityStatus> {
  std::vector<traffic_simulator::CanonicalizedEntityStatus>
      conflicting_entity_status;
  ...
  for (const auto &status : other_entity_status) {
    if (status.second.laneMatchingSucceed() &&
        std::count(conflicting_lanes.begin(), conflicting_lanes.end(),
                   status.second.getLaneletId()) >= 1) {
      conflicting_entity_status.emplace_back(status.second);
    }
  }
...
auto ActionNode::foundConflictingEntity(
    const lanelet::Ids &following_lanelets) const -> bool {
  auto conflicting_crosswalks =
      hdmap_utils->getConflictingCrosswalkIds(following_lanelets);
  auto conflicting_lanes =
      hdmap_utils->getConflictingLaneIds(following_lanelets);
  for (const auto &status : other_entity_status) {
    if (status.second.laneMatchingSucceed() &&
        std::count(conflicting_crosswalks.begin(), conflicting_crosswalks.end(),
                   status.second.getLaneletId()) >= 1) {
...
auto ActionNode::calculateUpdatedEntityStatus(
  ...
  if (const auto canonicalized_lanelet_pose =
    ...
    const auto distance = (twist_new.linear.x +
                           canonicalized_entity_status->getTwist().linear.x) /
                          2.0 * step_time;
    std::cout << "calculateUpdatedEntityStatus::distance: " << distance
              << std::endl;
    auto entity_status_updated = static_cast<traffic_simulator::EntityStatus>(
    ...
    std::cout << "calculateUpdatedEntityStatus::entity_status_updated.pose."
                 "position.x: "
              << entity_status_updated.pose.position.x << std::endl;
    return entity_status_updated;
  } else {
    THROW_SIMULATION_ERROR("Cannot move along lanelet - entity ",
                           std::quoted(canonicalized_entity_status->getName()),
...
auto ActionNode::calculateUpdatedEntityStatusInWorldFrame(
  ...
  pose_new.orientation = canonicalized_entity_status->getMapPose().orientation *
                         angular_trans_quat;
  ...
  pose_new.position.x =
      trans_vec(0) + canonicalized_entity_status->getMapPose().position.x;
  pose_new.position.y =
      trans_vec(1) + canonicalized_entity_status->getMapPose().position.y;
  pose_new.position.z =
      trans_vec(2) + canonicalized_entity_status->getMapPose().position.z;
  auto entity_status_updated = static_cast<traffic_simulator::EntityStatus>(
      *canonicalized_entity_status);
  entity_status_updated.time = current_time + step_time;
...
auto ActionNode::calculateStopDistance(
    const traffic_simulator_msgs::msg::DynamicConstraints &constraints) const
    -> double {
  return traffic_simulator::longitudinal_speed_planning::
      LongitudinalSpeedPlanner(step_time,
                               canonicalized_entity_status->getName())
          .getRunningDistance(0, constraints,
                              canonicalized_entity_status->getTwist(),
                              canonicalized_entity_status->getAccel(),
                              canonicalized_entity_status->getLinearJerk());
}
} // namespace entity_behavior
