...
auto EgoEntity::requestSpeedChange(
  const double target_speed, const SpeedChangeTransition,
  const SpeedChangeConstraint, const bool) -> void
{
...
{
  THROW_SEMANTIC_ERROR(
    "The traffic_simulator's request to set speed to the Ego type entity is for initialization purposes only.");
}
