...
auto EntityBase::getDistanceToSyncronizationPosition(
  const lanelet::Id & intersection_id // dont know about this, maybe i can get the other infomation?
  const double stop_position //may be if the info givven is only the intersection, perhaps needs info about how far the stop position is from the intersection
  ) -> double
{
...
auto EntityBase::adjustElementVelocityByIntersection(
  const double ego_distance_to_intersection,
