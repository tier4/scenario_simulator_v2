...
auto EntityManager::fillEgoLaneletPose(traffic_simulator_msgs::msg::EntityStatus & status) const
  -> void
{
  const auto ego_name = getEgoName();
  const auto ego_route = entities_.at(ego_name)->getRouteLanelets();
  const auto unique_route_lanelets = traffic_simulator::helper::getUniqueValues(ego_route);

  std::optional<traffic_simulator_msgs::msg::LaneletPose> lanelet_pose;

  if (unique_route_lanelets.empty()) {
    lanelet_pose = hdmap_utils_ptr_->toLaneletPose(
      status.pose, entities_.at(ego_name)->getStatus().bounding_box, false, 1.0);
  } else {
