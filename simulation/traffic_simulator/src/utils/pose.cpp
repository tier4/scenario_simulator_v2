  ...
  -> double
{
  if (const auto from_lanelet_pose = lanelet_wrapper::pose::toLaneletPose(
        from_pose, from_bounding_box, false, matching_distance);
      from_lanelet_pose.has_value()) {
    if (const auto to_lanelet_pose =
          lanelet_wrapper::pose::toLaneletPose(to_pose, to_bounding_box, false, matching_distance);
  ...
  return math::geometry::hypot(from_pose.position, to_pose.position);
}
