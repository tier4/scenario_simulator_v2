...
{
  return Spline(lanelet_map_core::lanelet_map::centerPoints(route_lanelets));
}
  ...
  const std::vector<CanonicalizedLaneletPose> & other_poses) -> bool
{
  ...
  auto conflicting_lanes = lanelet_map_core::lanelet_map::conflictingLaneIds(following_lanelets);
  ...
  const std::vector<CanonicalizedLaneletPose> & other_poses) -> bool
{
  ...
  const auto lanelet_ids_list =
    lanelet_map_core::lanelet_map::rightOfWayLaneletIds(following_lanelets);
  for (const auto & pose : other_poses) {
...
{
  return lanelet_map_core::pose::alongLaneletPose(
  ...
  -> std::vector<Point>
{
  ...
  Spline spline(lanelet_map_core::lanelet_map::centerPoints(ids));
  ...
  for (const auto id : ids) {
    if (id == lanelet_pose.lanelet_id) {
      ...
      s_in_spline = lanelet_map_core::lanelet_map::laneletLength(id) + s_in_spline;
    }
  }
  ...
  const lane_change::Parameter & parameter) -> LaneletPose
{
  ...
  switch (parameter.constraint.type) {
    ...
    case lane_change::Constraint::Type::LONGITUDINAL_DISTANCE:
      return lanelet_map_core::pose::alongLaneletPose(lanelet_pose, parameter.constraint.value);
  ...
  const lane_change::Parameter & parameter) -> std::optional<std::pair<Curve, double>>
{
  if (lanelet_map_core::lane_change::canChangeLane(
    ...
    switch (parameter.constraint.type) {
      ...
      */
      case lane_change::Constraint::Type::NONE:
        return lanelet_map_core::lane_change::laneChangeTrajectory(
          pose::toMapPose(lanelet_pose), parameter, 10.0, 20.0, 1.0);
      case lane_change::Constraint::Type::LATERAL_VELOCITY:
        return lanelet_map_core::lane_change::laneChangeTrajectory(lanelet_pose, parameter);
      case lane_change::Constraint::Type::LONGITUDINAL_DISTANCE:
        return lanelet_map_core::lane_change::laneChangeTrajectory(lanelet_pose, parameter);
      case lane_change::Constraint::Type::TIME:
        return lanelet_map_core::lane_change::laneChangeTrajectory(lanelet_pose, parameter);
  ...
  const lane_change::Parameter & parameter) -> std::vector<Point>
{
  if (const double rest_s = current_s + horizon - curve.getLength(); rest_s < 0) {
    ...
    const auto center_points = lanelet_map_core::lanelet_map::centerPoints(following_lanelets);
    // DIFFERENT SPLINE - recalculation needed
