...
std::int64_t HdMapUtils::getTrafficLightRelationId(std::int64_t lanelet_id) const
{
  std::vector<std::int64_t> ret;
  ...
  for (const auto light : autoware_traffic_lights) {
    for (auto light_string : light->lightBulbs()) {
      if (light_string.hasAttribute("traffic_light_id")) {
        ...
        if (id && id.get() == lanelet_id) {
          return light->id();
        }
      }
    }
  }
  THROW_SEMANTIC_ERROR("traffic_light_id : ", lanelet_id, " does not match to relation.");
}
