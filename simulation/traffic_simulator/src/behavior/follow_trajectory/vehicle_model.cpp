...
{
auto Vehicle::createUpdatedStatus(
  const geometry_msgs::msg::Vector3 & velocity, const double step_time) const
  -> traffic_simulator_msgs::msg::EntityStatus
{
  ...
  updated_status.action_status.twist.angular =
    quaternion_operation::convertQuaternionToEulerAngle(quaternion_operation::getRotation(
      status->pose.orientation, updated_status.pose.orientation)) /
    step_time;
...
{
  if (vehicle_parameters == nullptr) {
    throw common::Error(
      "Access to vehicle_parameters of Vehicle: ", status->name,
      ", but vehicle_parameters == nullptr");
  }
  return vehicle_parameters->axles.front_axle.max_steering;
}
...
auto Vehicle::getWheelBase() const -> double
{
  ...
  return vehicle_parameters->axles.front_axle.position_x;
}
...
auto Vehicle::createUpdatedStatus(const double steering, const double speed, const double step_time)
