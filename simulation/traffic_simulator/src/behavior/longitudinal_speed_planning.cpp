...
LongitudinalSpeedPlanner::LongitudinalSpeedPlanner(const double step_time,
...
auto LongitudinalSpeedPlanner::getAccelerationDuration(
  ...
  return getQuadraticAccelerationDuration(target_speed, constraints,
                                          current_twist, current_accel) +
         getLinearAccelerationDuration(target_speed, constraints, current_twist,
                                       current_accel);
}
...
auto LongitudinalSpeedPlanner::planConstraintsFromJerkAndTimeConstraint(
  ...
  const auto quad_duration = getQuadraticAccelerationDuration(
      target_speed, constraints, current_twist, current_accel);
  if (quad_duration >= acceleration_duration) {
    ret.max_acceleration_rate =
        2 *
        (target_speed - current_twist.linear.x -
         current_accel.linear.x * acceleration_duration) /
    ...
    if (isAccelerating(target_speed, current_twist)) {
      ...
      if (discriminant < 0) {
        THROW_SEMANTIC_ERROR("Negative discriminant: the target speed is too "
                             "high compared to the acceleration.");
      }

      ret.max_acceleration =
          constraints.max_acceleration_rate * acceleration_duration -
      ...
      if (discriminant < 0) {
        THROW_SEMANTIC_ERROR("Negative discriminant: the target speed is too "
                             "high compared to the deceleration.");
      }

      ret.max_deceleration =
          -constraints.max_deceleration_rate * acceleration_duration +
          std::sqrt(discriminant) + current_accel.linear.x;
    }
  }
...
auto LongitudinalSpeedPlanner::getDynamicStates(
    ...
    -> std::tuple<geometry_msgs::msg::Twist, geometry_msgs::msg::Accel,
                  double> {
  if (target_speed > constraints.max_speed) {
    target_speed = constraints.max_speed;
  }
  double linear_jerk =
      planLinearJerk(target_speed, constraints, current_twist, current_accel);
  std::cout << "linear_jerk: " << linear_jerk << " step: " << step_time
            << std::endl;
  ...
  std::cout << "accel: " << accel.linear.x << " twist: " << twist.linear.x
            << std::endl;
  std::cout << "linear_jerk: " << linear_jerk << std::endl;
...
auto LongitudinalSpeedPlanner::getRunningDistance(
  ...
  do {
    next_state =
        getDynamicStates(target_speed, constraints, std::get<0>(next_state),
  ...
  } while (!isTargetSpeedReached(target_speed, std::get<0>(next_state),
                                 twist_tolerance));
  return ret;
}
...
auto LongitudinalSpeedPlanner::getVelocityWithConstantJerk(
    ...
    const geometry_msgs::msg::Accel &current_accel,
    const traffic_simulator_msgs::msg::DynamicConstraints &constraints,
    double duration) const -> double {
  if (isAccelerating(target_speed, current_twist)) {
    return getVelocityWithConstantJerk(current_twist, current_accel,
...
auto LongitudinalSpeedPlanner::getQuadraticAccelerationDurationWithConstantJerk(
  ...
  if (isAccelerating(target_speed, current_twist)) {
    return (-current_accel.linear.x +
            std::sqrt(current_accel.linear.x * current_accel.linear.x +
                      2 * constraints.max_acceleration_rate *
...
auto LongitudinalSpeedPlanner::getQuadraticAccelerationDuration(
    ...
    const geometry_msgs::msg::Accel &current_accel) const -> double {
  if (isAccelerating(target_speed, current_twist)) {
    double duration = (constraints.max_acceleration - current_accel.linear.x) /
                      constraints.max_acceleration_rate;
    double v = getVelocityWithConstantJerk(current_twist, current_accel,
                                           constraints.max_acceleration_rate,
                                           duration);
    // While quadratic acceleration, the entity does not reached the target
    // speed.
    if (v < target_speed) {
      return duration;
    }
    ...
    double duration = (current_accel.linear.x - constraints.max_deceleration) /
                      constraints.max_deceleration_rate;
    double v = getVelocityWithConstantJerk(current_twist, current_accel,
                                           -constraints.max_deceleration_rate,
                                           duration);
...
auto LongitudinalSpeedPlanner::getLinearAccelerationDuration(
    ...
    const geometry_msgs::msg::Twist &current_twist,
    const geometry_msgs::msg::Accel &current_accel) const -> double {
  const double quad_duration = getQuadraticAccelerationDuration(
      target_speed, constraints, current_twist, current_accel);
  if (isReachedToTargetSpeedWithConstantJerk(target_speed, constraints,
                                             current_twist, current_accel,
                                             quad_duration)) {
    return 0;
  } else {
...
auto LongitudinalSpeedPlanner::isReachedToTargetSpeedWithConstantJerk(
  ...
  return (std::abs(getVelocityWithConstantJerk(target_speed, current_twist,
                                               current_accel, constraints,
                                               duration) -
...
auto LongitudinalSpeedPlanner::isAccelerating(
    double target_speed,
    const geometry_msgs::msg::Twist &current_twist) const -> bool {
...
auto LongitudinalSpeedPlanner::planLinearJerk(
  ...
  if (isAccelerating(target_speed, current_twist)) {
    accel_x_new = std::clamp(
        current_accel.linear.x + step_time * constraints.max_acceleration_rate,
        0.0,
        std::min(constraints.max_acceleration,
                 (target_speed - current_twist.linear.x) / step_time));
  } else {
    accel_x_new = std::clamp(
        current_accel.linear.x - step_time * constraints.max_deceleration_rate,
        std::max(constraints.max_deceleration * -1,
                 (target_speed - current_twist.linear.x) / step_time),
        0.0);
  }
...
auto LongitudinalSpeedPlanner::forward(
  ...
  ret.linear.x = std::clamp(ret.linear.x, constraints.max_deceleration * -1,
...
auto LongitudinalSpeedPlanner::forward(
  ...
  ret.linear.x = std::clamp(ret.linear.x, -1 * constraints.max_speed,
...
auto LongitudinalSpeedPlanner::timeDerivative(
    ...
    const geometry_msgs::msg::Twist &after) const -> geometry_msgs::msg::Accel {
  using math::geometry::operator-;
  using math::geometry::operator/;
...
auto LongitudinalSpeedPlanner::timeDerivative(
    const geometry_msgs::msg::Accel &before,
    const geometry_msgs::msg::Accel &after) const -> double {
