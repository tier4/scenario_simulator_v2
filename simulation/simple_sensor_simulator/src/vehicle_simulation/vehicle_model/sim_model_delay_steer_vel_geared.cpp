...
#include <simple_sensor_simulator/vehicle_simulation/vehicle_model/sim_model_delay_steer_vel_geared.hpp>

SimModelDelaySteerVelGeared::SimModelDelaySteerVelGeared(
  double vx_lim, double steer_lim, double vx_rate_lim, double steer_rate_lim, double wheelbase,
  double dt, double vx_delay, double vx_time_constant, double steer_delay,
  double steer_time_constant, double steer_dead_band, double debug_vx_scaling_factor,
...
: SimModelInterface(5 /* dim x */, 2 /* dim u */),
  ...
  debug_vx_scaling_factor_(std::max(debug_vx_scaling_factor, 0.0)),
  debug_steer_scaling_factor_(std::max(debug_steer_scaling_factor, 0.0))
{
  initializeInputQueue(dt);
}

double SimModelDelaySteerVelGeared::getX() { return state_(IDX::X); }
double SimModelDelaySteerVelGeared::getY() { return state_(IDX::Y); }
double SimModelDelaySteerVelGeared::getYaw() { return state_(IDX::YAW); }
double SimModelDelaySteerVelGeared::getVx() { return state_(IDX::VX); }
double SimModelDelaySteerVelGeared::getVy() { return 0.0; }
double SimModelDelaySteerVelGeared::getAx() { return current_ax_; }
double SimModelDelaySteerVelGeared::getWz()
{
  return state_(IDX::VX) * std::tan(state_(IDX::STEER)) / wheelbase_;
}
double SimModelDelaySteerVelGeared::getSteer() { return state_(IDX::STEER); }
void SimModelDelaySteerVelGeared::update(const double & dt)
{
...
void SimModelDelaySteerVelGeared::initializeInputQueue(const double & dt)
{
  size_t vx_input_queue_size = static_cast<size_t>(round(vx_delay_ / dt));
  vx_input_queue_.resize(vx_input_queue_size);
  std::fill(vx_input_queue_.begin(), vx_input_queue_.end(), 0.0);
...
Eigen::VectorXd SimModelDelaySteerVelGeared::calcModel(
  const Eigen::VectorXd & state, const Eigen::VectorXd & input)
{
  ...
  const double delay_vx_des = sat(delay_input_vx, vx_lim_, -vx_lim_) * debug_vx_scaling_factor_;
  const double vx_rate = sat(-(vx - delay_vx_des) / vx_time_constant_, vx_rate_lim_, -vx_rate_lim_);
  const double delay_steer_des = sat(delay_input_steer, steer_lim_, -steer_lim_) * debug_steer_scaling_factor_;
...
void SimModelDelaySteerVelGeared::updateStateWithGear(
  Eigen::VectorXd & state, const Eigen::VectorXd & prev_state, const uint8_t gear, const double dt)
{
  const auto setStopState = [&]() {
    ...
    current_ax_ = (state(IDX::VX) - prev_state(IDX::VX)) / std::max(dt, 1.0e-5);
  };
