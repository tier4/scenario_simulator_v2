...
#include <simple_sensor_simulator/vehicle_simulation/vehicle_model/sim_model_perfect_trajectory_follower.hpp>
...
SimModelPerfectTrajectoryFollower::SimModelPerfectTrajectoryFollower(double delay_time_sec)
: SimModelInterface(4 /* dim_x: x, y, yaw, vx */, 0 /* dim_u */),
  delay_time_sec_(delay_time_sec)
{
}

void SimModelPerfectTrajectoryFollower::setInitialReference(
...
void SimModelPerfectTrajectoryFollower::setTrajectory(
...
double SimModelPerfectTrajectoryFollower::getX()
{
  std::lock_guard<std::mutex> lock(mutex_);
  return state_(X);
}

double SimModelPerfectTrajectoryFollower::getY()
{
  std::lock_guard<std::mutex> lock(mutex_);
  return state_(Y);
}

double SimModelPerfectTrajectoryFollower::getYaw()
{
  std::lock_guard<std::mutex> lock(mutex_);
  return state_(YAW);
}

double SimModelPerfectTrajectoryFollower::getVx()
{
  std::lock_guard<std::mutex> lock(mutex_);
  return state_(VX);
}

double SimModelPerfectTrajectoryFollower::getAx()
{
  std::lock_guard<std::mutex> lock(mutex_);
  return current_ax_;
}

double SimModelPerfectTrajectoryFollower::getWz()
{
  std::lock_guard<std::mutex> lock(mutex_);
  return current_wz_;
}

double SimModelPerfectTrajectoryFollower::getSteer()
{
  std::lock_guard<std::mutex> lock(mutex_);
  return current_steer_;
}

double SimModelPerfectTrajectoryFollower::getZ() const
{
  std::lock_guard<std::mutex> lock(mutex_);
  return current_z_map_;
}

double SimModelPerfectTrajectoryFollower::getPitch() const
{
  std::lock_guard<std::mutex> lock(mutex_);
  return current_pitch_;
}

void SimModelPerfectTrajectoryFollower::update(const double & dt)
{
  std::lock_guard<std::mutex> lock(mutex_);
...
Eigen::VectorXd SimModelPerfectTrajectoryFollower::calcModel(
  const Eigen::VectorXd & /*state*/, const Eigen::VectorXd & /*input*/)
{
  return Eigen::VectorXd::Zero(4);
}

double SimModelPerfectTrajectoryFollower::interpolateZ(
...
double SimModelPerfectTrajectoryFollower::calculatePitchFromLanelet(
