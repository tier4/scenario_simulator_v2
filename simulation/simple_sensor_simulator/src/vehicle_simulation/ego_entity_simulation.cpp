...
void EgoEntitySimulation::initializeExternalMode()
{
  ...
  ego_odometry_sub_.emplace(
    ...
    [this](const nav_msgs::msg::Odometry & msg) { onKinematicState(msg); });

  // /initialpose3d is the only SS2 → Godot bridge for placing the ego at the scenario's
  // start pose. Autoware's /api/localization/initialize service targets Autoware's own
  // localization stack and does not reach Godot.
...
void EgoEntitySimulation::onKinematicState(const nav_msgs::msg::Odometry & msg)
{
  ...
  const double relative_yaw = math::geometry::convertQuaternionToEulerAngle(relative.orientation).z;
