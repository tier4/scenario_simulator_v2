...
lanelet::ConstLanelets query::laneletLayer(
    const lanelet::LaneletMapConstPtr& ll_map) {
  ...
  for (auto li = ll_map->laneletLayer.begin(); li != ll_map->laneletLayer.end();
       li++) {
...
lanelet::ConstLanelets query::subtypeLanelets(const lanelet::ConstLanelets lls,
                                              const char subtype[]) {
...
lanelet::ConstPolygons3d query::getAllParkingLots(
  ...
  for (const auto& poly : lanelet_map_ptr->polygonLayer) {
    const std::string type =
        poly.attributeOr(lanelet::AttributeName::Type, "none");
...
lanelet::ConstLineStrings3d query::getAllParkingSpaces(
  ...
  for (const auto& ls : lanelet_map_ptr->lineStringLayer) {
    const std::string type =
        ls.attributeOr(lanelet::AttributeName::Type, "none");
...
bool query::getLinkedLanelet(const lanelet::ConstLineString3d& parking_space,
  ...
  return query::getLinkedLanelet(parking_space, all_road_lanelets,
                                 all_parking_lots, linked_lanelet);
}
...
bool query::getLinkedLanelet(const lanelet::ConstLineString3d& parking_space,
  ...
  for (const auto& lanelet : linked_lanelets) {
    const double distance =
        boost::geometry::distance(to2D(parking_space).basicLineString(),
...
lanelet::ConstLanelets query::getLinkedLanelets(
  ...
  if (!getLinkedParkingLot(parking_space, all_parking_lots,
                           &linked_parking_lot)) {
    return linked_lanelets;
  }
  const auto& candidate_lanelets =
      getLinkedLanelets(linked_parking_lot, all_road_lanelets);
  ...
  for (const auto& lanelet : candidate_lanelets) {
    ...
    const Eigen::Vector3d new_pt =
        parking_space.front().basicPoint() - direction * distance_thresh;

    const lanelet::Point3d check_line_p1(lanelet::InvalId, new_pt.x(),
                                         new_pt.y(), new_pt.z());
    const lanelet::Point3d check_line_p2(lanelet::InvalId,
                                         parking_space.back().basicPoint());
    const lanelet::LineString3d check_line(lanelet::InvalId,
                                           {check_line_p1, check_line_p2});
...
std::vector<lanelet::ConstLineString3d> query::stopLinesLanelet(
  ...
  if (traffic_light_reg_elems.size() > 0) {
    ...
    for (auto j = traffic_light_reg_elems.begin();
         j < traffic_light_reg_elems.end(); j++) {
      lanelet::Optional<lanelet::ConstLineString3d> traffic_light_stopline_opt =
          (*j)->stopLine();
  ...
  if (traffic_sign_reg_elems.size() > 0) {
    ...
    for (auto j = traffic_sign_reg_elems.begin();
         j < traffic_sign_reg_elems.end(); j++) {
...
ConstLanelets query::getLaneletsWithinRange(
    const lanelet::ConstLanelets& lanelets,
    const geometry_msgs::msg::Point& search_point, const double range) {
...
ConstLanelets query::getLaneChangeableNeighbors(
  ...
  const auto lanelets = getLaneletsWithinRange(
  ...
  for (const auto& llt : lanelets) {
    ...
    road_slices.insert(road_slices.end(), tmp_road_slice.begin(),
                       tmp_road_slice.end());
  }
  return road_slices;
}
...
ConstLanelets query::getAllNeighborsRight(const routing::RoutingGraphPtr& graph,
                                          const ConstLanelet& lanelet) {
  ConstLanelets lanelets;
  auto right_lane = (!!graph->right(lanelet)) ? graph->right(lanelet)
                                              : graph->adjacentRight(lanelet);
  while (!!right_lane) {
    lanelets.push_back(right_lane.get());
    right_lane = (!!graph->right(right_lane.get()))
                     ? graph->right(right_lane.get())
                     : graph->adjacentRight(right_lane.get());
  }
  return lanelets;
}

ConstLanelets query::getAllNeighborsLeft(const routing::RoutingGraphPtr& graph,
                                         const ConstLanelet& lanelet) {
  ConstLanelets lanelets;
  auto left_lane = (!!graph->left(lanelet)) ? graph->left(lanelet)
                                            : graph->adjacentLeft(lanelet);
  while (!!left_lane) {
    lanelets.push_back(left_lane.get());
    left_lane = (!!graph->left(left_lane.get()))
                    ? graph->left(left_lane.get())
...
bool query::getClosestLanelet(const ConstLanelets& lanelets,
  ...
  if (closest_lanelet_ptr == nullptr) {
    lanelet::HdMapException(
        "argument closest_lanelet_ptr is null! Failed to find closest lanelet");
    return false;
  }
  ...
  lanelet::ConstLanelets candidate_lanelets;
  {
    ...
    for (const auto& llt : lanelets) {
      double distance = boost::geometry::comparable_distance(
          llt.polygon2d().basicPolygon(), search_point);
  ...
  // find by angle
  {
    ...
    for (const auto& llt : candidate_lanelets) {
      lanelet::ConstLineString3d segment =
          getClosestSegment(search_point, llt.centerline());
      double segment_angle =
          std::atan2(segment.back().y() - segment.front().y(),
                     segment.back().x() - segment.front().x());
...
std::vector<std::deque<lanelet::ConstLanelet>>
getSucceedingLaneletSequencesRecursive(const routing::RoutingGraphPtr& graph,
                                       const lanelet::ConstLanelet& lanelet,
                                       const double length) {
  ...
  for (const auto& next_lanelet : next_lanelets) {
    ...
    auto tmp_lanelet_sequences = getSucceedingLaneletSequencesRecursive(
        graph, next_lanelet, length - lanelet_length);
...
std::vector<std::deque<lanelet::ConstLanelet>>
getPreceedingLaneletSequencesRecursive(const routing::RoutingGraphPtr& graph,
                                       const lanelet::ConstLanelet& lanelet,
                                       const double length) {
  ...
  for (const auto& prev_lanelet : prev_lanelets) {
    ...
    auto tmp_lanelet_sequences = getPreceedingLaneletSequencesRecursive(
        graph, prev_lanelet, length - lanelet_length);
    for (auto& tmp_lanelet_sequence : tmp_lanelet_sequences) {
...
std::vector<lanelet::ConstLanelets> query::getSucceedingLaneletSequences(
  ...
  for (const auto& next_lanelet : next_lanelets) {
    ...
    for (const auto& lanelet_sequence : lanelet_sequences_deq) {
      lanelet_sequences_vec.emplace_back(lanelet_sequence.begin(),
