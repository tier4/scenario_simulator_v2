...
namespace
{
inline IdPair orderedIdPair(const Id id1, const Id id2)
{
  return (id1 < id2) ? IdPair(id1, id2) : IdPair(id2, id1);
}
}  // namespace
...
private:
  template<typename Func1, typename Func2>
  LaneChangeLanelets getLaneChangeLanelets(
    LaneChangeMap::iterator iter, Func1 && prev,
    Func2 && next)
  {
    ...
    result.first = utils::concatenate({predecessors.first, ConstLanelets{iter->first},
          followers.first});
    result.second = utils::concatenate({predecessors.second, ConstLanelets{iter->second.target},
          followers.second});
    return result;
  }
  ...
  LaneChangeLanelets getAdjacentLaneChangeLanelets(LaneChangeMap::iterator iter, Func1 && adjacent)
  {
    ...
    while (true) {
      ...
      if (iter == laneChanges_.end() || iter->second.visited ||
        iter->second.target != nextTargetLlt)
      {
        break;
      }
      iter->second.visited = true;
...
RoutingGraphBuilder::RoutingGraphBuilder(
...
ConstLanelets RoutingGraphBuilder::getPassableLanelets(
  const LaneletLayer & lanelets,
  const traffic_rules::TrafficRules & trafficRules)
{
  ConstLanelets llts;
...
ConstAreas RoutingGraphBuilder::getPassableAreas(
  const AreaLayer & areas,
  const traffic_rules::TrafficRules & trafficRules)
{
  ConstAreas ars;
...
void RoutingGraphBuilder::addEdges(
...
void RoutingGraphBuilder::addEdges(
  const ConstAreas & areas, const LaneletLayer & passableLanelets,
...
void RoutingGraphBuilder::addFollowingEdges(const ConstLanelet & ll)
{
  auto endPointsLanelets =
    pointsToLanelets_.equal_range(orderedIdPair(ll.leftBound().back().id(),
      ll.rightBound().back().id()));
  // Following
  ConstLanelets following;
  std::for_each(endPointsLanelets.first, endPointsLanelets.second, [&ll, this, &following](
      auto it) {
  ...
  std::for_each(endPointsLanelets.first, endPointsLanelets.second,
    [&following, this, &merging](auto it) {
      if (geometry::follows(it.second,
      following.front()) && this->trafficRules_.canPass(it.second, following.front()))
      {
...
void RoutingGraphBuilder::addSidewayEdge(
  LaneChangeLaneletsCollector & laneChangeLanelets, const ConstLanelet & ll,
  const ConstLineString3d & bound, const RelationType & relation)
{
  auto directlySideway = [&relation, &ll](const ConstLanelet & sideLl) {
      return relation ==
             RelationType::AdjacentLeft ? geometry::leftOf(sideLl, ll) : geometry::rightOf(sideLl,
               ll);
    };
...
void RoutingGraphBuilder::addConflictingEdge(
  ...
  const LaneletLayer & passableLanelets)
{
  ...
  for (auto & result : results) {
    ...
    if ((maxHeight &&
      geometry::overlaps3d(ll, other,
      *maxHeight)) || (!maxHeight && geometry::overlaps2d(ll, other)))
    {
      assignCosts(ll, other, RelationType::Conflicting);
      assignCosts(other, ll, RelationType::Conflicting);
    }
...
void RoutingGraphBuilder::addLaneChangeEdges(
  LaneChangeLaneletsCollector & laneChanges,
...
void RoutingGraphBuilder::assignLaneChangeCosts(
  const ConstLanelets & froms, const ConstLanelets & tos,
  const RelationType & relation)
{
  assert(relation == RelationType::Left || relation == RelationType::Right);
  assert(froms.size() == tos.size());
  auto costs = utils::transform(
    routingCosts_, [&](const RoutingCostPtr & cost) {
      return cost->getCostLaneChange(trafficRules_, froms, tos);
    });
  for (auto i = 0ul; i < froms.size(); ++i) {
    for (RoutingCostId costId = 0; costId < RoutingCostId(routingCosts_.size()); ++costId) {
      if (!std::isfinite(costs[costId])) {
        ...
        auto adjacent = relation ==
          RelationType::Left ? RelationType::AdjacentLeft : RelationType::AdjacentRight;
...
void RoutingGraphBuilder::assignCosts(
  ...
  const RelationType & relation)
{
  for (RoutingCostId rci = 0; rci < RoutingCostId(routingCosts_.size()); rci++) {
    ...
    if (relation == RelationType::Successor || relation == RelationType::Area) {
      ...
      edgeInfo.routingCost = routingCost.getCostLaneChange(trafficRules_,
          {*from.lanelet()}, {*to.lanelet()});
