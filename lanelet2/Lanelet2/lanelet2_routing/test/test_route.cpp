...
using AllRoutes =
  testing::Types<Route1, Route1NoLc, Route2, Route3, Route4, RouteMaxHoseLeftRight,
...
TYPED_TEST(AllRoutesTest, CheckValidity) {
  EXPECT_NO_THROW(this->route.checkValidity());
}                                                                                           // NOLINT
...
bool hasRelation(
  const LaneletRelations & relations, const ConstLanelet & llt,
...
TEST_F(RouteMaxHoseLeftRight, Lanes) {  // NOLINT
  ...
  EXPECT_TRUE(std::find(std::begin(fullLane), std::end(fullLane), ConstLanelet(lanelets.at(
      2024))) !=
    std::end(fullLane));
  EXPECT_EQ(route.numLanes(), 1ul);
}
...
TEST_F(GermanVehicleGraph, CannotCreateRoute) {  // NOLINT
  ...
  EXPECT_FALSE(!!route);
  // NOLINTNEXTLINE
  EXPECT_THROW(graph->getRoute(lanelets.at(2001), lanelets.at(
      2004), numCostModules), lanelet::InvalidInputError);
}
