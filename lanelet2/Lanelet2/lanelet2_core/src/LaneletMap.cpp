...
{
template<>
bool equals<lanelet::LineString3d, lanelet::LineString3d>(
  const lanelet::LineString3d & geometry1,
  const lanelet::LineString3d & geometry2)
{
  return geometry1 == geometry2;
}
template<>
bool equals<lanelet::Polygon3d, lanelet::Polygon3d>(
  const lanelet::Polygon3d & geometry1,
...
std::unordered_map<Id, T> toMap(const std::vector<T> & vec)
{
  ...
  auto elems = utils::transform(vec, [](auto & elem) {
        return typename Map::value_type(getId(elem), elem);
      });
  return std::unordered_map<Id, T>(std::make_move_iterator(elems.begin()),
...
auto getSecond(const std::vector<Pair> & p)
{
  std::vector<RType> v;
  v.reserve(p.size());
  std::transform(p.cbegin(), p.cend(), std::back_inserter(v), [](const auto & elem) {
      return elem.second;
    });
  return v;
}
...
{
  explicit AssignIdVisitor(LaneletMap * map)
  : map_(map) {}
...
{
  explicit AddVisitor(LaneletMap * map)
  : map_(map) {}
...
LaneletMapUPtr createMapFromConst(
...
template<>
void PrimitiveLayer<RegulatoryElementPtr>::add(
  ...
  const traits::ConstPrimitiveType<traits::OwnedT<PrimitiveLayer<T>::PrimitiveT>> & primitive) const
{
  return forEachMatchInMultiMap<traits::ConstPrimitiveType<typename PrimitiveLayer<T>::PrimitiveT>>(
    tree_->usage.ownedLookup, primitive, [](const auto & elem) {
...
{
  return forEachMatchInMultiMap<typename PrimitiveLayer<T>::PrimitiveT>(tree_->usage.ownedLookup,
           primitive,
...
template<typename T>
typename PrimitiveLayer<T>::ConstPrimitiveVec PrimitiveLayer<T>::search(const BoundingBox2d & area)
const
{
...
template<typename T>
typename PrimitiveLayer<T>::OptConstPrimitiveT PrimitiveLayer<T>::searchUntil(
...
template<typename T>
typename PrimitiveLayer<T>::OptPrimitiveT PrimitiveLayer<T>::searchUntil(
  const BoundingBox2d & area,
...
template<typename T>
typename PrimitiveLayer<T>::ConstPrimitiveVec PrimitiveLayer<T>::nearest(
...
template<typename T>
typename PrimitiveLayer<T>::PrimitiveVec PrimitiveLayer<T>::nearest(
  const BasicPoint2d & point,
  unsigned n)
{
  std::vector<typename Tree::TreeNode> nodes;
...
template<typename T>
typename PrimitiveLayer<T>::OptConstPrimitiveT PrimitiveLayer<T>::nearestUntil(
  const BasicPoint2d & point,
  const ConstSearchFunction & func) const
{
...
template<typename T>
typename PrimitiveLayer<T>::OptPrimitiveT PrimitiveLayer<T>::nearestUntil(
  const BasicPoint2d & point,
...
LaneletMapLayers::LaneletMapLayers(
...
{
  checkId(lanelet);
  utils::forEach(lanelet.regulatoryElements(), [&](auto & regElem) {
      this->trackParameters(*regElem);
    });
  laneletLayer.add(lanelet);
}
...
LaneletMapConstPtr LaneletSubmap::laneletMap() const
{
  ...
  auto addLs = [&map](auto & prim) {
      map->add(LineString3d(std::const_pointer_cast<LineStringData>(prim.constData()),
        prim.inverted()));
    };
...
{
  auto map =
    utils::createMap(Lanelets{laneletLayer.begin(), laneletLayer.end()},
      Areas{areaLayer.begin(), areaLayer.end()});
...
void LaneletSubmap::trackParameters(const RegulatoryElement & regelem)
{
  ...
  {
public:
    explicit AddToLLorAreaVisitor(LLorAreas & llOrAreas)
    : llOrAreas_{&llOrAreas} {}
...
template std::vector<ConstLayerPrimitive<Lanelet>> findUsages<Lanelet>(
  const PrimitiveLayer<Lanelet>&, Id);
template std::vector<ConstLayerPrimitive<LineString3d>> findUsages<LineString3d>(
  const PrimitiveLayer<LineString3d>&,
  Id);
template std::vector<ConstLayerPrimitive<Polygon3d>> findUsages<Polygon3d>(
...
ConstLanelets findUsagesInLanelets(const LaneletMapLayers & map, const ConstPoint3d & p)
{
  ...
  auto llts = utils::concatenate(ls, [&map](const auto & ls) {
        return map.laneletLayer.findUsages(ls);
      });
  auto lltsInv = utils::concatenate(ls, [&map](const auto & ls) {
        return map.laneletLayer.findUsages(ls.invert());
      });
...
ConstAreas findUsagesInAreas(const LaneletMapLayers & map, const ConstPoint3d & p)
{
  auto ls = map.lineStringLayer.findUsages(p);
  auto areas =
    utils::concatenate(ls, [&map](const auto & ls) {return map.areaLayer.findUsages(ls);});
  auto areasInv = utils::concatenate(ls, [&map](const auto & ls) {
        return map.areaLayer.findUsages(ls.invert());
      });
...
{
  return std::make_unique<LaneletMap>(LaneletLayer::Map(),
           AreaLayer::Map(), RegulatoryElementLayer::Map(),
           PolygonLayer::Map(), LineStringLayer::Map(), toMap(fromPoints));
}

LaneletMapUPtr createMap(const LineStrings3d & fromLineStrings)
{
  auto points = utils::concatenateRange(fromLineStrings, [](auto ls) {
...
LaneletMapUPtr createMap(const Lanelets & fromLanelets, const Areas & fromAreas)
{
  ...
  auto points = utils::concatenateRange(ls, [](auto ls) {
        return std::make_pair(ls.begin(), ls.end());
      });
  ...
  auto map = std::make_unique<LaneletMap>(toMap(fromLanelets), toMap(
        fromAreas), RegulatoryElementLayer::Map(),
...
{
  auto points = utils::concatenateRange(fromPolygons, [](auto ls) {
        return std::make_pair(ls.begin(), ls.end());
      });
  return std::make_unique<LaneletMap>(LaneletLayer::Map(),
           AreaLayer::Map(), RegulatoryElementLayer::Map(),
...
{
  return std::make_unique<LaneletSubmap>(LaneletLayer::Map(),
           AreaLayer::Map(), RegulatoryElementLayer::Map(),
           PolygonLayer::Map(), LineStringLayer::Map(), toMap(fromPoints));
}
...
{
  return createSubmap(fromLanelets, {});
}
...
{
  auto map = std::make_unique<LaneletSubmap>(toMap(fromLanelets), toMap(
        fromAreas), RegulatoryElementLayer::Map(),
...
{
public:
  explicit NSmallestElements(size_t n)
  : n_{n} {values_.reserve(n);}
  bool insert(double measure, T value)
  {
    auto pos =
      std::lower_bound(values_.begin(), values_.end(), measure, [](auto & v1, double v2) {
          return v1.first < v2;
        });
...
// clang-format off
// NOLINTNEXTLINE
#define INSTANCIATE_FIND_NEAREST(PRIM) template std::vector<std::pair<double, \
    PRIM>> findNearest<PRIM>(PrimitiveLayer<PRIM>&, const BasicPoint2d &, unsigned)
// NOLINTNEXTLINE
#define INSTANCIATE_CONST_FIND_NEAREST(PRIM, \
    CPRIM) template std::vector<std::pair<double, \
    CPRIM>> findNearest<PRIM>(const PrimitiveLayer<PRIM>&, const BasicPoint2d &, unsigned)
// clang-format on
