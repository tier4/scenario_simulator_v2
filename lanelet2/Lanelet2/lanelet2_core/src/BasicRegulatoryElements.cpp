...
{
bool operator==(
  const lanelet::LaneletDataConstWptr & lhs,
  const lanelet::LaneletDataConstWptr & rhs)
{
...
{
  return utils::transform(primitives, [](const auto & elem) {
...
void updateTrafficSigns(TrafficSignsWithType trafficSigns)
{
  if (!trafficSigns.type.empty()) {
    for (auto & sign : trafficSigns.trafficSigns) {
      sign.applyVisitor([](auto prim) {
          prim.setAttribute(AttributeName::Type, AttributeValueString::TrafficSign);
        });
      sign.applyVisitor([&](auto prim) {
          prim.setAttribute(AttributeName::Subtype, trafficSigns.type);
        });
    }
  }
}

RegulatoryElementDataPtr constructTrafficLightData(
...
RegulatoryElementDataPtr constructTrafficSignData(
  Id id, const AttributeMap & attributes,
...
RegulatoryElementDataPtr constructSpeedLimitData(
  Id id, const AttributeMap & attributes,
  const TrafficSignsWithType & trafficSigns,
  const TrafficSignsWithType & cancellingTrafficSigns,
  const LineStrings3d & refLines, const LineStrings3d & cancelLines)
{
  auto data = constructTrafficSignData(id, attributes, trafficSigns, cancellingTrafficSigns,
      refLines, cancelLines);
  data->attributes[AttributeName::Subtype] = AttributeValueString::SpeedLimit;
  return data;
}
RegulatoryElementDataPtr constructRightOfWayData(
  Id id, const AttributeMap & attributes, const Lanelets & rightOfWay,
...
RegulatoryElementDataPtr constructAllWayStopData(
  Id id, const AttributeMap & attributes,
  ...
  const LineStringsOrPolygons3d & signs)
{
  ...
  RuleParameterMap rpm = {
    {RoleNameString::Yield, llts}, {RoleNameString::RefLine, sl},
    {RoleNameString::Refers, toRuleParameters(signs)}};
  auto data = std::make_shared<RegulatoryElementData>(id, std::move(rpm), attributes);
  data->attributes[AttributeName::Type] = AttributeValueString::RegulatoryElement;
...
TrafficLight::TrafficLight(const RegulatoryElementDataPtr & data)
...
Optional<LineString3d> TrafficLight::stopLine()
{
...
LineStringsOrPolygons3d TrafficLight::trafficLights()
{
  return getLsOrPoly(parameters(), RoleName::Refers);
}
...
void TrafficLight::setStopLine(const LineString3d & stopLine)
{
...
RightOfWay::RightOfWay(const RegulatoryElementDataPtr & data)
: RegulatoryElement(data)
{
...
ConstLanelets RightOfWay::rightOfWayLanelets() const
{
  return getParameters<ConstLanelet>(RoleName::RightOfWay);
}

Lanelets RightOfWay::rightOfWayLanelets()
{
  return utils::strong(getParameters<WeakLanelet>(RoleName::RightOfWay));
}

ConstLanelets RightOfWay::yieldLanelets() const
{
  return getParameters<ConstLanelet>(RoleName::Yield);
}

Lanelets RightOfWay::yieldLanelets()
{
  return utils::strong(getParameters<WeakLanelet>(RoleName::Yield));
}
...
void RightOfWay::setStopLine(const LineString3d & stopLine)
{
  parameters()[RoleName::RefLine] = {stopLine};
}
...
void RightOfWay::addYieldLanelet(const Lanelet & lanelet)
{
  parameters()[RoleName::Yield].emplace_back(lanelet);
}
...
TrafficSign::TrafficSign(const RegulatoryElementDataPtr & data)
: RegulatoryElement{data}
{
...
: TrafficSign(
    constructTrafficSignData(id, attributes, trafficSigns, cancellingTrafficSigns, refLines,
    cancelLines)) {}
...
LineStringsOrPolygons3d TrafficSign::trafficSigns()
{
  return getLsOrPoly(parameters(), RoleName::Refers);
}
...
std::string TrafficSign::type() const
{
  ...
  {
    const auto & attr = signs.front().applyVisitor([](auto & prim) {
          return prim.attribute(AttributeName::Subtype);
        });
    return attr.value();
  }
  if (!signs.empty()) {
...
ConstLineStrings3d TrafficSign::refLines() const
{
...
void TrafficSign::addRefLine(const LineString3d & line)
{
  parameters()[RoleName::RefLine].emplace_back(line);
}
...
SpeedLimit::SpeedLimit(
  ...
  const LineStrings3d & cancelLines)
: TrafficSign(
    constructSpeedLimitData(id, attributes, trafficSigns, cancellingTrafficSigns, refLines,
    cancelLines)) {}

SpeedLimit::SpeedLimit(const RegulatoryElementDataPtr & data)
: TrafficSign(data) {}
...
LineStringsOrPolygons3d TrafficSign::cancellingTrafficSigns()
{
  return getLsOrPoly(parameters(), RoleName::Cancels);
}
...
std::vector<std::string> TrafficSign::cancelTypes() const
{
  ...
  for (auto & sign : signs) {
    types.push_back(sign.applyVisitor([](auto & prim) {
        return prim.attribute(AttributeName::Subtype);
      }).value());
  }
...
ConstLineStrings3d TrafficSign::cancelLines() const
{
  return getParameters<ConstLineString3d>(RoleName::CancelLine);
}
...
ConstLineStrings3d AllWayStop::stopLines() const
{
  return getParameters<ConstLineString3d>(RoleName::RefLine);
}
...
LineStringsOrPolygons3d AllWayStop::trafficSigns()
{
  return getLsOrPoly(parameters(), RoleName::Refers);
}
...
AllWayStop::AllWayStop(const RegulatoryElementDataPtr & data)
: RegulatoryElement{data}
{
