...
public:
  ...
  {
    const bool result = intersectsLeft(seg) || intersectsRight(seg) || crossesEntry(seg) ||
      crossesExit(seg);
    return result;
  }
...
private:
  template<typename Func>
  static void forEachNearestPointUntilImpl(
    ConstLineString2d::const_iterator iter,
    const PointTree & tree, Func && f)
  {
    for (auto qIt =
      tree.qbegin(boost::geometry::index::nearest(iter->basicPoint(), unsigned(tree.size())));
      qIt != tree.qend(); ++qIt)
    {
      if (f(qIt->second)) {
        break;
  ...
  static std::vector<BasicSegment2d> makeSegments(const ConstLineString2d & line)
  {
    ...
    auto makeSegment = [](const auto & seg) {
  ...
  const BoundChecker & bounds, const Point3d & lastPoint, bool isLeft)
{
  ...
  auto nonintersectingPointLoop = [&](ConstLineString2d::const_iterator candidate) {
      ...
      if (!bounds.intersects(centerlineCandidate) &&
        !bounds.secondCrossesBounds(boundConnection, isLeft) &&
...
std::shared_ptr<ConstLineString3d> calculateCenterline(
  ...
  const ConstLineString2d & rightBound)
{
  ...
  while (leftCurrent != leftBound.end() || rightCurrent != rightBound.end()) {
    ...
    std::tie(leftCandidate, leftCandidateDistance) =
      findClosestNonintersectingPoint(std::next(
          leftCurrent), rightCurrent, bounds, centerlinePoints.back(), true);

    // Determine right candidate
    std::tie(rightCandidate, rightCandidateDistance) =
      findClosestNonintersectingPoint(std::next(
          rightCurrent), leftCurrent, bounds, centerlinePoints.back(), false);
    // Choose the better one
    if (leftCandidateDistance &&
      (!rightCandidateDistance || leftCandidateDistance <= rightCandidateDistance))
    {
    ...
    } else if (rightCandidateDistance &&
      (!leftCandidateDistance || leftCandidateDistance > rightCandidateDistance))
    {
...
void lanelet::LaneletData::resetCache() const
{
  if (!hasCustomCenterline()) {
    std::atomic_store_explicit(&centerline_,
      std::shared_ptr<ConstLineString3d>(), std::memory_order_release);
  }
}
...
bool has(const ConstLanelet & ll, Id id)
{
  ...
  return has(ll.leftBound(), id) || has(ll.rightBound(), id) ||
         std::any_of(regelems.begin(), regelems.end(), [&id](const auto & elem) {
             return elem->id() == id;
           });
}
}  // namespace utils
