...
struct point_mutable_traits<lanelet::ConstPoint2d>
{
  ...
  static inline void set(
    lanelet::ConstPoint2d & /*point*/, const orientation_2d & /*orient*/,
...
struct point_mutable_traits<lanelet::BasicPoint2d>
{
  ...
  static inline void set(
    lanelet::BasicPoint2d & /*point*/, const orientation_2d & /*orient*/,
    VoronoiPrecision /*value*/)
  {
    assert(false && "boost polygon should never call this!");
  }
...
struct point_traits<AlignedBasicPoint2d>
{
  using coordinate_type = VoronoiPrecision;  // NOLINT
  static inline coordinate_type get(
    const AlignedBasicPoint2d & point,
    const orientation_2d & orient)
  {
    return toVoronoi((orient == HORIZONTAL) ? point.x() : point.y());
  }
...
struct point_mutable_traits<AlignedBasicPoint2d>
{
  ...
  static inline void set(
    AlignedBasicPoint2d & /*point*/, const orientation_2d & /*orient*/,
    VoronoiPrecision /*value*/)
  {
    assert(false && "boost polygon should never call this!");
  }
...
bool isConnectionConvex(
  const BasicPoint2d & seg1, const BasicPoint2d & seg2,
...
PolygonIteratorPair getAlignedIterators(const IndexedPolygon & poly, const IndexedSegment & segment)
{
  ...
  if (itPair.first == poly.end() || itPair.second == poly.end()) {
    throw InvalidInputError("Index " + std::to_string(segment.first) + "-" +
...
IndexedPolygon removeSegment(const IndexedPolygon & poly, const IndexedSegment & excluded)
{
  ...
  IndexedPolygon res(std::next(itP.second),
    std::next(itP.first) == poly.end() ? itP.first : poly.end());
  ...
  const IndexedSegment & border)
{
  ...
  if (!noWrap) {
    poly.insert(std::next(npIt.second, std::distance(std::next(mergeIt.second),
      toMerge.end())), toMerge.begin(),
      mergeIt.first);
  }
}

MergeResult merge(
  const IndexedPolygon & poly, const IndexedPolygon & toMerge,
  const IndexedSegment & border)
{
...
MetricSegmentPair getMetricAdjacentSegmentPair(
  const BasicPolygon2d & metricPolygon, const IndexedPolygon & poly,
  const IndexedSegment & segment)
{
  const auto newSegments = getAdjacent(poly, segment);
  const BasicPoint2d newFirst = metricPolygon.at(newSegments.first.second) - metricPolygon.at(
    newSegments.first.first);
...
{
  return isConnectionConvex(-local.first, extension.second) && isConnectionConvex(-extension.first,
           local.second);
}
...
Connections findGates(
  const BasicPolygon2d & metricPolygon, const Gates & candidates, const Visited & overallVisited,
  const SegmentationData & segData)
{
  ...
  for (const auto & gate : candidates) {
    ...
    gates.emplace_back(Connection{gate.border, getOpposingAngle(metricSegPair,
        opposingPoly.size()), metricSegPair,
        opposingPolygonIdx});
  }
...
void updateState(
  Head & head, TraversalResult & traversalResult, const IndexedPolygon & newPoly,
  ...
  const SegmentationData & segData, const bool isOrigin)
{
  ...
  for (const auto & conn : gates) {
    ...
    const auto headsideAdjacent =
      getMetricAdjacentSegmentPair(metricPolygon, head.poly, conn.border);
...
std::vector<IndexedPolygon> mergeTriangulation(
...
void subdivide(IndexedTriangles & target, const IndexedPolygon & poly)
{
  ...
  for (auto first = std::next(poly.begin()), second = std::next(poly.begin(), 2);
    second != poly.end();
    ++second, ++first)
  {
...
SegmentationData makeVoronoi(const BasicPolygon2d & poly)
{
  ...
  auto advance =
    [](const VoronoiGraph::edge_type * & edge,
      VoronoiGraph::cell_type::source_index_type & lastIndex,
