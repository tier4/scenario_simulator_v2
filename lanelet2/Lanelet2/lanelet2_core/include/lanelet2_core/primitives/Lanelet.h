...
public:
   ...
   */
  LaneletData(
    Id id, LineString3d leftBound, LineString3d rightBound,
...
public:
  ...
  ConstLanelet(
    Id id, const LineString3d & leftBound, const LineString3d & rightBound,
    const AttributeMap & attributes = AttributeMap(),
    const RegulatoryElementPtrs & regulatoryElements = RegulatoryElementPtrs())
  : ConstPrimitive{std::make_shared<LaneletData>(id, leftBound, rightBound, attributes,
        regulatoryElements)} {}
...
public:
  ...
  {
    return inverted() ? data()->rightBound().invert() : data()->leftBound();
  }
  ...
  {
    return inverted() ? data()->leftBound().invert() : data()->rightBound();
  }
