...
{
  using Iterator = internal::TransformIterator<internal::ReverseAndForwardIterator<Points3d::iterator>,
      PointT>;
};
template<typename PointT>
...
{
  using Iterator =
    internal::TransformIterator<internal::ReverseAndForwardIterator<Points3d::const_iterator>,
      const PointT>;
};
template<>
...
inline Points3d::const_iterator pointIter(
...
public:
  ...
  {
    return inverted ? IteratorT(const_iterator(points_.rbegin())) : IteratorT(const_iterator(points_
             .begin()));
  }
  ...
  {
    return inverted ? IteratorT(const_iterator(points_.rend())) : IteratorT(const_iterator(points_.
             end()));
  }
  template<typename IteratorT = iterator>
...
public:
  ...
  explicit ConstLineStringImpl(
    Id id = InvalId, Points3d points = Points3d(),
  ...
  {
    return constData()->template begin<const_iterator>(inverted());
  }
  ...
  {
    return constData()->template begin<BasicIterator>(inverted_);
  }
  ...
  {
    return constData()->template end<BasicIterator>(inverted_);
  }
...
public:
  ...
  template<typename OtherT>
  explicit LineStringImpl(const LineStringImpl<OtherT> & other)
  : Base(other) {}

  explicit LineStringImpl(const std::shared_ptr<const LineStringData> &, bool inverted) = delete;

  //! Construct from linestring data
  explicit LineStringImpl(const std::shared_ptr<LineStringData> & data, bool inverted)
  : Base(data, inverted) {}
  ...
  LineStringImpl & operator=(std::vector<PointType> rhs)
  {
    if (inverted()) {
      points() =
        utils::transform(rhs.rbegin(), rhs.rend(), [](const auto & v) {return Point3d(v);});
    } else {
  ...
  {
    inverted() ? void(points().insert(points().begin(), Point3d(point))) : points().push_back(Point3d(
        point));
  }
...
public:
  ...
  explicit ConstHybridLineString3d(const ConstLineString3d & ls)
  : ConstLineString3d(ls) {}
  explicit ConstHybridLineString3d(const LineString3d & ls)
  : ConstLineString3d(ls) {}

  //! Returns an inverted linestring, O(0)
  ConstHybridLineString3d invert() const noexcept
  {
    return ConstHybridLineString3d{constData(), !inverted()};
  }
...
public:
  ...
  explicit ConstHybridLineString2d(const ConstLineString2d & ls)
  : ConstLineString2d(ls) {}
  explicit ConstHybridLineString2d(const LineString2d & ls)
  : ConstLineString2d(ls) {}

  //! Returns an inverted linestring, O(0)
  ConstHybridLineString2d invert() const noexcept
  {
    return ConstHybridLineString2d{constData(), !inverted()};
  }
...
bool operator==(
  const ConstLineStringImpl<LhsPointT> & lhs,
