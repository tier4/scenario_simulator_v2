...
public:
  ...
  : min_(other.min()), max_(other.max()) {}                                                                  // NOLINT
  ...
  {
    return {min_.cwiseMin(b.min_), max_.cwiseMax(b.max_)};
  }
  ...
  inline explicit BoundingBox2d(
    const Eigen::AlignedBox<OtherScalarType,
    AmbientDimAtCompileTime> & other)
  {
...
inline BoundingBox3d to3D(const BoundingBox2d & primitive)
{
  return {to3D(primitive.min()), to3D(primitive.max())};
}
inline BoundingBox2d to2D(const BoundingBox3d & primitive)
{
  return {to2D(primitive.min()), to2D(primitive.max())};
}
}  // namespace traits
}  // namespace lanelet
