...
IfPoly<Polygon3dT, std::pair<BasicPoint3d, BasicPoint3d>> projectedBorderPoint3d(
  const Polygon3dT & l1,
  const Polygon3dT & l2)
{
  static_assert(traits::is3D<Polygon3dT>(), "Please call this function with a 3D type!");
  return internal::projectedBorderPoint3d(traits::toHybrid(traits::to3D(l1)),
...
IfPoly<Polygon3dT, double> distanceToBorder3d(const Polygon3dT & poly1, const Polygon3dT & poly2)
{
  static_assert(traits::is3D<Polygon3dT>(), "Please call this function with a 3D type!");
  auto projPoint =
    internal::projectedBorderPoint3d(traits::toHybrid(poly1), traits::toHybrid(poly2));
...
IfPoly<Polygon2dT, bool> overlaps2d(const Polygon2dT & poly1, const Polygon2dT & poly2)
{
  ...
  using Mask = boost::geometry::detail::relate::static_mask<'T', '*', '*', '*', '*', '*', '*', '*',
      '*'>;
  return boost::geometry::detail::relate::relate<Mask>(
    utils::toHybrid(poly1),
    utils::toHybrid(poly2));
#endif
}

template<typename Polygon3dT>
IfPoly<Polygon3dT, bool> overlaps3d(
...
template<>
inline IfPoly<BasicPolygon2d, BasicPolygons2d> convexPartition<BasicPolygon2d>(
...
template<>
inline IfPoly<BasicPolygon2d, IndexedTriangles> triangulate<BasicPolygon2d>(
  const BasicPolygon2d & poly)
{
