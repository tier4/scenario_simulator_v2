...
constexpr bool isTrivialDistance()
{
  return traits::noRegelem<T1>() && traits::noRegelem<T2>() &&
         !(traits::isCategory<T1, traits::LineStringTag>() && traits::isCategory<T2,
...
{
  return distance(internal::GetGeometry<Geometry1T>::twoD(
             p1), internal::GetGeometry<Geometry2T>::twoD(p2));
}
...
->std::enable_if_t<internal::isTrivialDistance<Geometry1T, Geometry2T>(), double>
{
  return distance(internal::GetGeometry<Geometry1T>::threeD(
             p1), internal::GetGeometry<Geometry2T>::threeD(p2));
}
}  // namespace geometry
}  // namespace lanelet
