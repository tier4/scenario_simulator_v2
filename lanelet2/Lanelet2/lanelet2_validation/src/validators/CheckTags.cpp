...
const ValueMap & knownPointValues()
{
  ...
  const static ValueMap Tags{{Attr::Type,
    {Value::Dot, Value::End, Value::Pole, Value::Post, Value::Start}}};
  return Tags;
}
...
const ValueMap & knownLineStringValues()
{
  ...
  const static ValueMap Tags{
    ...
    {Str(Value::Arrow) + Attr::Subtype,
      {Value::Left, Value::LeftRight, Value::Right, Value::Straight, Value::StraightLeft,
        Value::StraightRight}},
...
const ValueMap & knownLaneletValues()
{
  ...
  const static ValueMap Tags{
    ...
    {Attr::Subtype,
      {Value::BicycleLane, Value::BusLane, Value::Crosswalk, Value::EmergencyLane, Value::Highway,
        Value::PlayStreet,
...
const ValueMap & knownAreaValues()
{
  ...
  const static ValueMap Tags{
    ...
    {Attr::Subtype,
      {Value::BicycleLane, Value::Building, Value::BusLane, Value::Crosswalk, Value::EmergencyLane,
        Value::Exit,
        Value::Freespace, Value::Highway, Value::Keepout, Value::Parking, Value::PlayStreet,
        Value::Road,
        Value::SharedWalkway, Value::Stairs, Value::TrafficIsland, Value::Vegetation,
        Value::Walkway}},
    {Attr::Location, {Value::Nonurban, Value::Private, Value::Urban}}};
  return Tags;
}
...
Issues checkKnownTags(
  const AttributeMap & attrs, const std::vector<std::string> & knownTags, Id id,
  Primitive primitive)
{
  Issues issues;
  for (const auto & attr : attrs) {
    bool isKnown = utils::anyOf(knownTags, [&attr](auto & knownTag) {
          return startsWith(attr.first, knownTag);
        });
    if (!isKnown) {
      issues.push_back(
        Issue(Severity::Warning, primitive, id,
        "attribute " + attr.first + " is not a known lanelet2 tag."));
    }
  }
  return issues;
}

Issues checkMandatoryTags(
  const AttributeMap & attrs, const std::vector<std::string> & mandatoryTags, Id id,
  Primitive primitive)
{
  Issues issues;
...
Issues checkKnownTagValues(
  const AttributeMap & attrs, const ValueMap & knownTagValues, Id id, Primitive primitive,
  bool combineSubtypes = false)
{
  ...
  for (const auto & attr : attrs) {
    ...
    bool isKnown = std::binary_search(values->second.begin(),
        values->second.end(), attr.second.value());
    if (!isKnown) {
...
Issues checkAttribute(const AttributeMap & map, Id id, Primitive primitive)
{
  ...
  for (const auto & attr : map) {
    bool isBoolAttr =
      utils::anyOf(boolAttributes, [&attr](auto & boolAttr) {
          return startsWith(attr.first, boolAttr);
        });
...
Issues UnknownTags::operator()(const lanelet::LaneletMap & map)
{
  ...
  append(issues, utils::concatenate(map.lineStringLayer, [](auto & elem) {
      return checkKnownTags(elem.attributes(), knownLineStringTags(), elem.id(),
  ...
  append(issues, utils::concatenate(map.regulatoryElementLayer, [](auto & elem) {
      return checkKnownTags(elem->attributes(), knownRegelemTags(), elem->id(),
...
Issues UnknownTagValue::operator()(const lanelet::LaneletMap & map)
{
  Issues issues;
  append(issues, utils::concatenate(map.pointLayer, [](auto & elem) {
      return checkKnownTagValues(elem.attributes(), knownPointValues(), elem.id(),
      Primitive::Point);
    }));
  append(issues, utils::concatenate(map.lineStringLayer, [](auto & elem) {
      return checkKnownTagValues(elem.attributes(), knownLineStringValues(), elem.id(),
      Primitive::LineString,
      true);
    }));
  append(issues, utils::concatenate(map.laneletLayer, [](auto & elem) {
      return checkKnownTagValues(elem.attributes(), knownLaneletValues(), elem.id(),
      Primitive::Lanelet);
    }));
  append(issues, utils::concatenate(map.areaLayer, [](auto & elem) {
...
Issues MandatoryTags::operator()(const lanelet::LaneletMap & map)
{
  Issues issues;
  append(issues, utils::concatenate(map.lineStringLayer, [](auto & elem) {
      return checkMandatoryTags(elem.attributes(), mandatoryLineStringTags(), elem.id(),
      Primitive::LineString);
    }));
  append(issues, utils::concatenate(map.laneletLayer, [](auto & elem) {
      return checkMandatoryTags(elem.attributes(), mandatoryLaneletTags(), elem.id(),
      Primitive::Lanelet);
    }));
  append(issues, utils::concatenate(map.areaLayer, [](auto & elem) {
