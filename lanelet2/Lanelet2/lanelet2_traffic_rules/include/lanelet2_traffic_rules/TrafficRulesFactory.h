...
public:
  ...
  void registerStrategy(
    const std::string & location, const std::string & participant,
   ...
   */
  static TrafficRulesUPtr create(
    const std::string & location, const std::string & participant,
...
public:
  RegisterTrafficRules(const std::string & location, const std::string & participant)
  {
    ...
    TrafficRulesFactory::instance().registerStrategy(
      ...
      +[] (const TrafficRules::Configuration & config)->TrafficRulesUPtr {return std::make_unique<T>(
          config);});
  }
};
}  // namespace traffic_rules
