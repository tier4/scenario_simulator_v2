...
Optional<std::shared_ptr<lanelet::routing::Route>> getRouteWrapper(
  const lanelet::routing::RoutingGraph & self,
...
Optional<std::shared_ptr<lanelet::routing::Route>> getRouteViaWrapper(
  const lanelet::routing::RoutingGraph & self,
...
routing::RoutingGraphPtr makeRoutingGraph(
  LaneletMap & laneletMap, const traffic_rules::TrafficRules & trafficRules,
  const routing::RoutingCostPtrs & routingCosts)
{
  return routing::RoutingGraph::build(laneletMap, trafficRules, routingCosts);
}

routing::RoutingGraphPtr makeRoutingGraphSubmap(
  LaneletSubmap & laneletMap,
...
BOOST_PYTHON_MODULE(PYTHON_API_MODULE_NAME) {  // NOLINT
  ...
  auto possPCost =
    static_cast<LaneletPaths (RoutingGraph::*)(const ConstLanelet &, double, RoutingCostId,
      bool) const>(
    &RoutingGraph::possiblePaths);
  auto possPLen =
    static_cast<LaneletPaths (RoutingGraph::*)(const ConstLanelet &, uint32_t, bool,
      RoutingCostId) const>(
    &RoutingGraph::possiblePaths);
  auto possPToCost =
    static_cast<LaneletPaths (RoutingGraph::*)(const ConstLanelet &, double, RoutingCostId,
  ...
  .add_property("predecessor", &LaneletVisitInformation::predecessor,
  ...
  .def("routingRelation", &RoutingGraph::routingRelation,
  ...
  .def("adjacentLefts", &RoutingGraph::adjacentLefts, "all left (non-routable) lanelets",
    arg("lanelet"))
  .def("adjacentRights", &RoutingGraph::adjacentRights, "all right (non-routable) lanelets",
    arg("lanelet"))
  .def("leftRelations", &RoutingGraph::leftRelations, "relations to left lanelets", arg("lanelet"))
  .def("rightRelations", &RoutingGraph::rightRelations, "relations to right lanelets", arg(
      "lanelet"))
  .def("conflicting", &RoutingGraph::conflicting, "Conflicting lanelets", arg("lanelet"))
  .def("reachableSet", &RoutingGraph::reachableSet,
    "set of lanelets that can be reached from a given lanelet",
    (arg("lanelet"), arg("maxRoutingCost"), arg("RoutingCostId") = 0, arg(
      "allowLaneChanges") = true))
  .def("reachableSetTowards", &RoutingGraph::reachableSetTowards,
    "set of lanelets that can reach a given lanelet",
    (arg("lanelet"), arg("maxRoutingCost"), arg("RoutingCostId") = 0, arg(
      "allowLaneChanges") = true))
  .def("possiblePaths", possPCost,
    "possible paths from a given start lanelet that are 'minRoutingCost'-long",
    (arg("lanelet"), arg("minRoutingCost"), arg("RoutingCostId") = 0,
    arg("allowLaneChanges") = false,
    arg("routingCostId") = 0))
  .def("possiblePathsTowards", possPToCost,
    ...
    arg("allowLaneChanges") = false,
    arg("routingCostId") = 0))
  .def("possiblePathsMinLen", possPLen,
  ...
  .def("forEachSuccessor",
    ...
    self.forEachSuccessor(from, std::move(
      func), lc, costId);
  },
  ...
  .def("forEachSuccessorIncludingAreas",
    +[] (RoutingGraph & self, const ConstLaneletOrArea & from, object func, bool lc,
  ...
  .def("forEachPredecessor",
    +[] (RoutingGraph & self, const ConstLanelet & from, object func, bool lc, RoutingCostId costId) {
    self.forEachPredecessor(from, std::move(
      func), lc, costId);
  },
  ...
  .def("forEachPredecessorIncludingAreas",
    +[] (RoutingGraph & self, const ConstLaneletOrArea & from, object func, bool lc,
    RoutingCostId costId) {
    self.forEachPredecessorIncludingAreas(from, std::move(
      func), lc, costId);
  },
    ...
    (arg("lanelet"), arg("func"), arg("allowLaneChanges") = true, arg("routingCostId") = 0))
  .def("exportGraphML", +[] (RoutingGraph & self, const std::string & path) {self.exportGraphML(
        path);},
    "Export the internal graph to graphML (xml-based) file format")
  .def("exportGraphViz",
    +[] (RoutingGraph & self, const std::string & path) {self.exportGraphViz(path);},
  ...
  .def("passableLaneletSubmap", &RoutingGraph::passableSubmap,
  ...
  .def("laneletSubmap",
    +[] (const Route & r) {return std::const_pointer_cast<LaneletSubmap>(r.laneletSubmap());},
  ...
  .def("followingRelations", &Route::followingRelations,
    "Provides the following lanelets within the Route")
  .def("previousRelations", &Route::previousRelations,
  ...
  .def("leftRelations", &Route::leftRelations,
  ...
  .def("rightRelations", &Route::rightRelations,
    "*all* lanelets right of a given lanelet within the Route")
  .def("conflictingInRoute", &Route::conflictingInRoute,
  ...
  .def("forEachSuccessor",
    +[] (Route & self, const ConstLanelet & from,
    object func) {self.forEachSuccessor(from, std::move(
        func));},
    "calls a function on each successor of lanelet with increasing cost. The function must receives a "
    "LaneletVisitInformation object as input and must return a bool whether followers of the current lanelet "
    "should be visited as well. The function can raise an exception if an early exit is desired",
  ...
  .def("forEachPredecessor",
    +[] (Route & self, const ConstLanelet & from,
    object func) {self.forEachPredecessor(from, std::move(func));},
    "similar to forEachSuccessor but instead goes backwards along the routing graph",
    (arg("lanelet"), arg("func")))
