...
struct Origin
{
  explicit Origin(const GPSPoint & position)
  : position{position}, isDefault{false} {}
  Origin() = default;
...
inline DefaultProjector defaultProjection(Origin origin = Origin::defaultOrigin())
{
  return DefaultProjector(origin);
}
}  // namespace lanelet
