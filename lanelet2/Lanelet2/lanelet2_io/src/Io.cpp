...
std::unique_ptr<LaneletMap> load(
  const std::string & filename, const Origin & origin, ErrorMessages * errors,
...
std::unique_ptr<LaneletMap> load(
  const std::string & filename, const Projector & projector, ErrorMessages * errors,
  const io::Configuration & params)
{
  ...
  ErrorMessages err;
  auto map =
    io_handlers::ParserFactory::createFromExtension(extension(filename), projector, params)->parse(
    filename, err);
...
std::unique_ptr<LaneletMap> load(
  const std::string & filename, const std::string & parserName, const Origin & origin,
  ...
  ErrorMessages * errors, const io::Configuration & params)
{
  ...
  ErrorMessages err;
  auto map =
    io_handlers::ParserFactory::create(parserName, projector, params)->parse(filename, err);
  handleErrorsOrThrow<ParseError>(err, errors);
  return map;
}
...
void write(
  const std::string & filename, const LaneletMap & map, const Origin & origin,
...
{
  ErrorMessages err;
  io_handlers::WriterFactory::createFromExtension(extension(filename), projector, params)->write(
    filename, map, err);
