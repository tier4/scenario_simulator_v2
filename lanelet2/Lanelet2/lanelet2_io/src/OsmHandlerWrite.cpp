...
void removeAndFixPlaceholders(
  osm::Primitive ** toRemove, osm::Roles & fromRoles,
  std::vector<UnresolvedRole> & placeholders)
{
  // find other placeholders that we have to fix
  auto remIt = std::find_if(fromRoles.begin(), fromRoles.end(), [&](auto & role) {
...
{
public:
  static std::unique_ptr<osm::File> writeMap(
    const LaneletMap & laneletMap, const Projector & projector,
    ErrorMessages & errors)
  {
    ...
    auto unparsedLaneletAndAreaParameters = writer.appendRegulatoryElements(
      laneletMap.regulatoryElementLayer);
...
private:
  ...
  void writeNodes(const LaneletMap & map, const Projector & projector)
  {
    ...
    for (const auto & point : map.pointLayer) {
      try {
        ...
        osmNodes.emplace(point.id(),
          osm::Node(point.id(), getAttributes(point.attributes()), gpsPoint));
  ...
  void appendLanelets(const LaneletLayer & laneletLayer)
  {
    for (const auto & lanelet : laneletLayer) {
      ...
      auto & insertedRelation =
        file_->relations.emplace(id, osm::Relation(id, attributes)).first->second;
      ...
      if (lanelet.hasCustomCenterline()) {
        tryInsertMembers(members, RoleNameString::Centerline,
          lanelet.centerline().id(), file_->ways, id);
      }
      for (const auto & regElem : lanelet.regulatoryElements()) {
        tryInsertMembers(members, RoleNameString::RegulatoryElement,
          regElem->id(), file_->relations, id);
      }
    }
  }
  ...
  void appendAreas(const AreaLayer & areaLayer)
  {
    for (const auto & area : areaLayer) {
      ...
      auto innerIds = utils::concatenate(area.innerBoundPolygons(), [](const auto & elem) {
            return elem.ids();
          });
      auto & members = insertedRelation.members;
    ...
    for (const auto & param : unparsedMembers) {
      ...
      try {
        ...
        *param.location = &relations.at(param.referencedRoleId);
        ...
        removeAndFixPlaceholders(param.location, relations.at(
            param.relationId).members, unparsedMembers);
      }
    }
  }
  ...
  void writeOsmWay(const PrimT & mapWay, osm::Ways & osmWays)
  {
    ...
    try {
      const auto wayNodes =
        utils::transform(mapWay, [ & nodes = file_->nodes](const auto & elem) {
            return &nodes.at(elem.id());
          });
  ...
  {
public:
    WriteRegulatoryElementVisitor(osm::File & file, ToFileWriter & writer)
    ...
    void operator()(const ConstPoint3d & p) override
    {
      try {
        ...
        writer.writeError(
          id, "Regulatory element has parameters that are not in the point layer: "s + std::to_string(
            p.id()));
      }
    }
    void operator()(const ConstLineString3d & l) override
    {
      try {
        ...
        writer.writeError(
          id, "Regulatory element has parameters that are not in the line string layer: "s + std::to_string(
            l.id()));
      }
    }
    void operator()(const ConstPolygon3d & p) override
    {
      try {
        ...
        writer.writeError(
          id, "Regulatory element has parameters that are not in the polygon layer: "s + std::to_string(
            p.id()));
      }
    }
  ...
  void tryInsertMembers(
    osm::Roles & insertMembers, const char * insertRole, Id insertId, PrimitiveMap & primitiveMap,
    Id relationId)
  {
    try {
      insertMembers.emplace_back(insertRole, &primitiveMap.at(insertId));
    } catch (std::out_of_range &) {
      writeError(relationId, "Relation has a member with id "s + std::to_string(
          insertId) + " that is not in the map!");
    }
  }
...
void testAndPrintLocaleWarning(ErrorMessages & errors)
{
  ...
  if (decimalPoint == nullptr || *decimalPoint != '.') {
    ...
    (decimalPoint ==
    nullptr ? ' ' : *decimalPoint) << "\". This will lead to invalid osm output!\n";
...
std::unique_ptr<osm::File> OsmWriter::toOsmFile(
