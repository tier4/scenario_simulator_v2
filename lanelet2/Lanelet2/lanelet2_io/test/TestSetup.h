...
{
  return rhs.id == lhs.id && lhs.attributes == rhs.attributes &&
         rhs.parameters.size() == rhs.parameters.size();
}
inline bool operator==(const LaneletData & lhs, const LaneletData & rhs)
{
  ...
  return rhs.id == lhs.id && rhs.attributes == lhs.attributes &&
         ...
         *lhs.rightBound().constData() == *rhs.rightBound().constData() &&
         lhsRegelem.size() == lhsRegelem.size() &&
...
inline bool operator==(const LaneletMap & rhs, const LaneletMap & lhs)
{
  return lhs.pointLayer == rhs.pointLayer && lhs.lineStringLayer == rhs.lineStringLayer &&
         lhs.polygonLayer == rhs.polygonLayer &&
         lhs.regulatoryElementLayer == rhs.regulatoryElementLayer &&
...
inline Point3d setUpPoint(
  int & num, int xOffset = 0,
  const std::string & type = AttributeValueString::Start)
{
  return Point3d(getId(num), xOffset, num, 1, AttributeMap{{AttributeNamesString::Type, type}});
}

inline LineString3d setUpLineString(
  int & num,
  const std::string & type = AttributeValueString::Curbstone)
{
...
{
  RuleParameterMap map{{RoleNameString::Refers, {setUpLineString(num,
...
{
  return RegulatoryElementFactory::create(GenericRegulatoryElement::RuleName, getId(
             num), RuleParameterMap{});
}
...
inline bool hasId(const Ids & ids, Id id)
{
