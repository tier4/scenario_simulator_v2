...
namespace scenario_runner { inline namespace reader
{
  ...
  void callWithElements(const Node& parent,
  ...
  decltype(auto) callWithElement(const pugi::xml_node& parent, const std::string& name, Callee&& call_with)
  {
  ...
  {                                                                            \
    std::stringstream ss {};                                                   \
    ss << "given class \'" << child.name() << "\' (element of class \'" << PARENT.name() << "\') is valid OpenSCENARIO element, but is not supported"; \
    throw SyntaxError { ss.str() };                                            \
  }
