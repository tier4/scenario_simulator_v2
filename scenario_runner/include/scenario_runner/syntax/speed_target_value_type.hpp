...
namespace scenario_runner { inline namespace syntax
{
  ...
  std::basic_istream<Ts...>& operator >>(std::basic_istream<Ts...>& is, SpeedTargetValueType& type)
  {
    ...
    {                                                                          \
      std::stringstream ss {};                                                 \
      ss << "given value \'" << buffer << "\' is valid OpenSCENARIO value of type SpeedTargetValueType, but it is not supported"; \
  ...
  template <typename... Ts>
  std::basic_ostream<Ts...>& operator <<(std::basic_ostream<Ts...>& os, const SpeedTargetValueType& type)
  {
    switch (type)
    {
    ...
    default:
      std::stringstream ss {};
      ss << "enum class SpeedTargetValueType holds unexpected value " << static_cast<SpeedTargetValueType::value_type>(type);
      throw ImplementationFault { ss.str() };
    }
  }
