...
namespace scenario_runner { inline namespace syntax
{
  ...
  template <typename... Ts>
  std::basic_ostream<Ts...>& operator <<(std::basic_ostream<Ts...>& os, const RelativeDistanceType& type)
  {
    switch (type)
    {
      #define BOILERPLATE(IDENTIFIER) case RelativeDistanceType::IDENTIFIER: return os << #IDENTIFIER;
    ...
    default:
      std::stringstream ss {};
      ss << "enum class RelativeDistanceType holds unexpected value " << static_cast<RelativeDistanceType::value_type>(type.value);
