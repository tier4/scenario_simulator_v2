...
namespace scenario_runner { inline namespace syntax
{
  ...
  template <typename... Ts>
  std::basic_istream<Ts...>& operator >>(std::basic_istream<Ts...>& is, TriggeringEntitiesRule& rule)
  {
    std::string buffer {};
  ...
  template <typename... Ts>
  std::basic_ostream<Ts...>& operator <<(std::basic_ostream<Ts...>& os, const TriggeringEntitiesRule& rule)
  {
    switch (rule)
    {
      #define BOILERPLATE(IDENTIFIER) case TriggeringEntitiesRule::IDENTIFIER: return os << #IDENTIFIER;
    ...
    default:
      std::stringstream ss {};
      ss << "enum class TriggeringEntitiesRule holds unexpected value " << static_cast<TriggeringEntitiesRule::value_type>(rule.value);
