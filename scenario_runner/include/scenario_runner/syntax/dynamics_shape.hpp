...
namespace scenario_runner { inline namespace syntax
{
  ...
  std::basic_istream<Ts...>& operator >>(std::basic_istream<Ts...>& is, DynamicsShape& shape)
  {
    ...
    {                                                                          \
      std::stringstream ss {};                                                 \
      ss << "given value \'" << buffer << "\' is valid OpenSCENARIO value of type DynamicsShape, but it is not supported"; \
  ...
  std::basic_ostream<Ts...>& operator <<(std::basic_ostream<Ts...>& os, const DynamicsShape& shape)
  {
    ...
    default:
      std::stringstream ss {};
      ss << "enum class DynamicsShape holds unexpected value " << static_cast<DynamicsShape::value_type>(shape);
      throw ImplementationFault { ss.str() };
    }
  }
