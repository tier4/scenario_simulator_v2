...
namespace scenario_runner { inline namespace syntax
{
  ...
  std::basic_istream<Ts...>& operator >>(std::basic_istream<Ts...>& is, VehicleCategory& category)
  {
    ...
    {                                                                          \
      std::stringstream ss {};                                                 \
      ss << "given value \'" << buffer << "\' is valid OpenSCENARIO value of type VehicleCategory, but it is not supported"; \
  ...
  template <typename... Ts>
  std::basic_ostream<Ts...>& operator <<(std::basic_ostream<Ts...>& os, const VehicleCategory& category)
  {
    switch (category)
    {
    ...
    default:
      std::stringstream ss {};
      ss << "enum class VehicleCategory holds unexpected value " << static_cast<VehicleCategory::value_type>(category);
      throw ImplementationFault { ss.str() };
    }
  }
