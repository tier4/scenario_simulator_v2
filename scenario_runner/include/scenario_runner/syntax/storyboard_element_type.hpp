...
namespace scenario_runner { inline namespace syntax
{
  ...
  template <typename... Ts>
  std::basic_ostream<Ts...>& operator <<(std::basic_ostream<Ts...>& os, const StoryboardElementType& type)
  {
    switch (type)
    {
      #define BOILERPLATE(IDENTIFIER) case StoryboardElementType::IDENTIFIER: return os << #IDENTIFIER;
    ...
    default:
      std::stringstream ss {};
      ss << "enum class StoryboardElementType holds unexpected value " << static_cast<StoryboardElementType::value_type>(type.value);
