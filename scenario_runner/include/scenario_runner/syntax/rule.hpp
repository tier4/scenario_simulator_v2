...
namespace scenario_runner { inline namespace syntax
{
  ...
  struct Rule
  {
    ...
    constexpr decltype(auto) operator ()(T&& lhs, U&& rhs) const noexcept
    {
      ...
      {
      case greaterThan:
        return std::greater<void>()(std::forward<decltype(lhs)>(lhs), std::forward<decltype(rhs)>(rhs));

      case lessThan:
        return std::less<void>()(std::forward<decltype(lhs)>(lhs), std::forward<decltype(rhs)>(rhs));
