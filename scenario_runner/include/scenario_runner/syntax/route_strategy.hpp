...
namespace scenario_runner { inline namespace syntax
{
  ...
  std::basic_istream<Ts...>& operator >>(std::basic_istream<Ts...>& is, RouteStrategy& strategy)
  {
    ...
    {                                                                          \
      std::stringstream ss {};                                                 \
      ss << "given value \'" << buffer << "\' is valid OpenSCENARIO value of type RouteStrategy, but it is not supported"; \
  ...
  template <typename... Ts>
  std::basic_ostream<Ts...>& operator <<(std::basic_ostream<Ts...>& os, const RouteStrategy& strategy)
  {
    switch (strategy)
    {
    ...
    default:
      std::stringstream ss {};
      ss << "enum class RouteStrategy holds unexpected value " << static_cast<std::underlying_type<RouteStrategy>::type>(strategy);
      throw ImplementationFault { ss.str() };
    }
  }
