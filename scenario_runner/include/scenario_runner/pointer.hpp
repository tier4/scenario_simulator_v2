...
#define DEBUG() std::cout << green << __FILE__ << magenta << ":" << cyan << __LINE__ << reset << std::endl
...
namespace scenario_runner
{
  ...
  public:
    ...
    decltype(auto) as() const
    {
      ...
      else // TODO more precise error report
      {
        std::stringstream ss {};
        ss << "type-error: can't treat " << binding().type().name() << " as type " << typeid(U).name();
